...
//
// 키는 ERROR_CODES 상수에서 계산하고 satisfies로 전수를 강제한다 — 계약에 코드가
// 추가되거나 키가 오타 나면 컴파일이 잡는다. 조용히 FALLBACK으로 새지 않는다.
//
// 원칙 (시안 S10 계열에서 확정):
...
import { ERROR_CODES, UNKNOWN_CODE, type ErrorCode } from "./contracts";
...
const MESSAGES = {
  ...
  [ERROR_CODES.INVALID_STATE_TRANSITION]: {
    title: "예약 상태가 바뀌었습니다",
    body: "화면에 보이던 상태와 지금 상태가 다릅니다. 최신 상태를 다시 확인했습니다.",
  },
  ...
  [ERROR_CODES.REQUEST_IN_PROGRESS]: {
    title: "예약을 처리하고 있습니다",
    body: "조금 전 요청이 아직 처리 중입니다. '다시 시도'를 누르면 같은 요청의 결과를 확인합니다 — 중복으로 잡히지 않습니다.",
  },
...
} satisfies Record<ErrorCode, ScreenMessage>;
...
// 세 화면이 반복하던 "ApiError면 코드로, 아니면 UNKNOWN으로" 패턴을 한 곳으로.
export function messageForError(e: unknown): ScreenMessage {
  if (e instanceof ApiError) return messageFor(e.code, e.traceId);
  return messageFor(UNKNOWN_CODE);
}
