...
F01의 결정 일부가 관리자 승인 전이고, 기각되면 계약이 바뀐다.

| 결정 | 기각되면 | 대응 | 현재 |
|---|---|---|---|
| D7 `confirmationCode` | 경로가 `/{code}` → `/{id}` 정수로 | `CODE_BASED_PATH=false` | 미정 — 플래그 유지 |
| D1 `guestCount` | 요청 본문에서 필드 제거 | `SEND_GUEST_COUNT=false` | **채택**(2026-08-15) |
| D4 `NO_SHOW` | 상태 하나가 사라짐 | 플래그로 격리해뒀음 | **기각**(2026-08-15) — 아래 |

시나리오 스크립트는 URL을 직접 만들지 않고 `pathFor()`만 쓴다. 그래서 계약이 뒤집혀도 고칠 곳이 한 파일이다.

**D4는 실제로 기각됐고, 이 대비책이 값을 냈다.** NO_SHOW를 쓰는 코드를 플래그 하나로 격리해둔 덕에 반영이 표 정의 한 곳 수정으로 끝났다. 다만 **플래그 자체는 지웠다.** 다시 켤 일이 없는 스위치를 남겨두면 읽는 사람이 "언젠가 켜지는 기능"으로 오해한다. 전이 표는 이제 **36칸(상태 6 × 이벤트 6) = 허용 7 + 멱등 6 + 거부 23**이다.
