...
`verify/common.sql`은 비용이 거의 없으니 매번 돌린다. 그중 **(3)번이 특히 값지다** — 이력에 남은 전이 조합이 허용 8행의 부분집합인지 보는 쿼리로, **"부하 중 발생한 N건의 전이가 전부 표 안에 있었다"**를 운영 데이터로 증명한다. 전수 테스트가 "코드가 거부하도록 짜여 있다"를 보인다면 이건 "실제로 한 건도 새지 않았다"를 보인다.
