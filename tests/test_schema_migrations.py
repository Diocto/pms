...
class Test_T29_시드_계약:
    ...
    def test_호텔_계약_전체를_고정한다(self, engine):
        """시드 계약 — hotel 표를 (id, name)까지 통째로 고정한다."""
    ...
    def test_객실타입_계약_전체를_고정한다(self, engine):
        """시드 계약 — room_type 표를 스펙 1.9절 (3) 그대로 전체 컬럼 단위로
        고정한다."""
