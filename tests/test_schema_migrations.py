...
def test_T27_예약코어_마이그레이션이_전부_적용됐다(engine):
    """현재 리비전의 조상에 052가 있는지 본다.

    `version_num == "052"` 단정은 "052가 전체 체인의 head다"라서, 선착순 특가가
    `201_…`을 붙이는 순간 예약 코어 스키마는 멀쩡한데 빨개진다 (리뷰 지적).
    검증할 것은 "예약 코어 것이 적용됐다"이므로 조상 관계로 본다.
    """
...
class Test_T29_시드_계약:
    """스펙 1.9절의 숫자 그대로. 검색·부하테스트·화면가 이 값을 상수로 박는다.

    2026-08-16 확장(리비전 053): 호텔 3~100이 추가됐다. 호텔 1·2와
    객실타입 1~5의 기존 계약은 **한 글자도 바뀌지 않는다** — 부하테스트의
    ...
    def test_기존_객실타입_계약_전체를_고정한다(self, engine):
        ...
        고정한다."""
        # 스펙 1.9절 (3) 표 그대로. capacity가 틀리면 검색의 정원
    ...
    def test_확장_호텔은_이름과_주소가_id에서_유도된다(self, engine):
        ...
        # 유도된다. 규칙에서 벗어난 행이 하나라도 있으면 화면의 상수 생성이
        # 조용히 어긋난다.
        #
