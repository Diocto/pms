...
`GET /api/internal/config`는 예약 코어가 라우트를 소유하지만, 싣는 내용은 컨텍스트마다
자기 것이 있다 — 예약 코어의 락·결제, 선착순 특가의 프로모션, 검색의 캐시. 엔드포인트를
컨텍스트마다 따로 파면 "설정을 두 곳에서 따로 읽는" 상태가 되고, 예약 코어가 다른
컨텍스트의 컨테이너를 직접 읽으면 의존 방향이 뒤집힌다.

그래서 계약을 여기(`common` — 아무에게도 의존하지 않는다)에 두고, 각 컨텍스트가
자기 컨테이너에 `RuntimeContributor` 구현 하나를 등록하며, 예약 코어의 라우트가 리스트
