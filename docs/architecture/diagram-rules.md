...
- **다이어그램 바로 아래에 "그림 해설" 문단을 한국어로 쓴다.** 그림 속 각 요소가 무엇이고, 관계 표기(화살표·점선·다이아몬드)가 무슨 뜻인지 풀어 쓴다. 그림만 던져두지 않는다. 코드 식별자(클래스명, 컬럼명)는 영어 그대로 두되, 처음 등장할 때 한국어 이름을 병기한다 (예: Reservation(예약서)).
