...
| 설정 노출 엔드포인트 경로 | **경로는 `GET /api/internal/config`로 유지 확정** (2026-08-15 — 우리 스크립트가 물고 있어 이름이 `RuntimeReport`로 바뀌어도 경로는 안 바뀜). 세 컨텍스트 설정 + `counters`가 한 응답에 담긴다 — 실행 전 확인이 GET 한 번 | 응답 스키마(특히 워커 수 포함 여부)는 F01 구현 때 확정. `reset.sh`의 `CONFIG_URL`로 덮어쓸 수 있다 |
