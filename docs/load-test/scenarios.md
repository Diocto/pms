...
구현에서 주의한 것 하나. **표본은 200 응답마다 무조건 넣는다.** "`source` 필드가 있을 때만 넣기"로 짜면 필드명이 어긋났을 때(예: `ApiModel` 미상속으로 `snake_case`) 표본이 0건이 되는데, **k6는 표본 없는 지표의 임계값을 조용히 통과시킨다.** 게이트가 정확히 울어야 하는 상황에서 게이트가 사라지는 것이다. 필드가 없으면 `false`로 넣어 rate가 0으로 잡히게 했다.
...
| 설정 노출 엔드포인트 경로 | **미확정** — F01 스펙 기준 `GET /api/internal/config`, 구현 전. **공용 `ConfigReporter`로 세 컨텍스트(F01·F02·F03) 설정이 한 응답에 담긴다**(2026-08-15) — 실행 전 확인이 GET 한 번으로 끝난다 | F01 계약 문서가 진실. 확정되면 PM이 전달. `reset.sh`의 `CONFIG_URL`로 덮어쓸 수 있다 |
