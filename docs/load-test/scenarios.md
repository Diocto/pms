...
**락 실패만 상태 코드가 아니라 본문 `code`로 판정한다 (2026-08-15 변경).** 병합된 앱의 예외→상태 표(`app/common/error_handlers.py`)에는 **400·404·409뿐이고 503 자리가 없다.** 락 실패가 409로 나오면 재고 소진과 상태 코드만으로는 구분되지 않고, 우리는 409를 "정당한 거절"로 빼므로 **락의 가격이 ON·OFF 양쪽 0으로 나온다.** 그러면 "락은 공짜"라는 거짓 결론이 리포트에 실린다. `code`는 앱이 계약으로 못박은 값이라(`errors.py`) 상태 코드보다 단단한 기준이기도 하다. 자세한 것은 §7 D1의 「구현 시 수정」.
...
부하를 넣기 전에 **우리가 적어둔 계약과 실제로 병합된 코드가 같은지** 확인한다. 넷 다 확인하고 넘어간다.
...
| 4 | **설정 노출 경로·키** | 실제 엔드포인트 응답 ↔ `reset.sh`의 `INFO_URL`·`LOCK_KEY` | **시작 자체가 막힌다.** `reset.sh`가 읽지 못하면 부하를 넣지 않는다 |
...
| **락 실패의 HTTP 상태 코드** | **미확정 — 409일 가능성이 높다** | `error_handlers.py`의 예외→상태 표에 **400·404·409 셋뿐이고 503이 없다.** 아래 참조 |
| **DB 커넥션 풀 크기** | 미확정 | 지연 수치를 해석하려면 필요하다 (§3-8). F01이 엔진을 만들 때 정해진다 |
| 설정 노출 엔드포인트 경로 | **미확정** — F01 스펙 기준 `GET /api/internal/config`, 구현 전 | `reset.sh`의 `CONFIG_URL`로 덮어쓸 수 있다 |
...
| 무엇 | 상태 |
|---|---|
| 설정 노출 엔드포인트 (`/actuator/info` 없음) | **여전히 확정 대기.** F01 스펙 기준 `GET /api/internal/config`이고 아직 구현 전이다. 앱에 있는 경로는 `/health` 하나. 응답 계약과 규칙은 그대로, 경로 한 줄만 바뀐다 — `CONFIG_URL`로 덮어쓸 수 있게 해뒀다 |
