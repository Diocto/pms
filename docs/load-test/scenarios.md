...
| **S5** | **락 스위치가 실제로는 안 꺼졌다.** 두 회차가 같은 조건이라 "차이 없음"이 나온다 | **`reset.sh s5on` / `s5off`가 `/actuator/env/pms.lock.enabled`를 읽어 기대값과 다르면 중단한다 (구현됨).** 읽지 못해도 중단한다 — 확인 못 한 대조는 대조가 아니다 |
...
**이 확인은 자동화돼 있다.** `reset.sh s5on` / `reset.sh s5off`가 `/actuator/env/pms.lock.enabled`를 읽어 기대값과 다르면 **부하를 넣지 않고 중단한다.** 엔드포인트를 읽지 못하는 경우에도 중단한다 — 확인하지 못한 대조는 대조가 아니기 때문이다. actuator env가 안 열려 있으면 F01에 열어달라고 요청한다.
