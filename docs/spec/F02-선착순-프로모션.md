...
F01의 예약 생성 유스케이스가 이 훅들을 `List<...>`로 주입받아 전부 호출한다. **구현이 0개면 빈 리스트라 아무 일도 일어나지 않는다.** 그래서 F02 병합 전에도 F01은 그대로 돌고, F02가 잘려나가도(00 D4 절단 3순위) F01은 멀쩡하다. F02는 빈 하나를 등록하면 자동으로 들어온다.
...
| C4 | 트랜잭션 안 순서: ① 일반 재고 차감 ② `Reservation` INSERT ③ 생성 훅 호출. **훅에서 예외를 던지면 ①②까지 전부 롤백된다** | **확정** |
| C5 | 시드 데이터 계약 — 재고 2026-08-01~10-29 고정, 객실타입 5종 | **확정** (§9-3) |
| **C6** | 예약 생성 입력(주문서)에 **특가 요청과 적용 단가를 넘길 자리**가 있는가 | **미확정.** §13 V4. 대체안은 아래 |

**C6이 없을 때의 대체안.** 생성 훅은 `CreateReservationCommand`를 받는다. 그 안에 "이 예약은 특가 요청이다"라는 표시가 없으면, 훅은 어떤 예약이 특가인지 구분할 수 없다. 이때 F02는 **자기 요청 범위 안에서만 사는 표시자**(request-scope 빈)를 두고, F02의 특가 API가 그것을 세운 뒤 F01을 부른다. 훅은 같은 스레드에서 동기 호출되므로 표시자를 읽을 수 있다. F01을 한 줄도 고치지 않는다는 점에서는 안전하지만, **호출 경로에 숨은 결합이 생기고 비동기 경로로 바뀌면 깨진다.** C6이 있으면 이 우회가 사라진다. 그래서 V4가 남은 재확인 중 1순위다.
...
| 4 | — | F01 예약 생성 유스케이스 호출. **락 획득도 트랜잭션도 F01이 연다** | 밖 → 안 |
| 5 | RoomDailyInventory (해당 1일 행) | `remaining` 1 감소 | **단일 트랜잭션** (F01) |
| 6 | Reservation | `PENDING` 상태로 새로 생성 → `reservationId` 확정 | 〃 |
...
| 9 | Redis 잔여 카운터 | 트랜잭션이 롤백됐으면 `INCR`로 되돌린다 | 커밋/롤백 후 (F02) |
| 10 | Redis 멱등 키 | 결과 저장 (TTL 24시간) | 밖 (F02) |
...
| 일반 재고 부족 (F01) | 409 `INSUFFICIENT_INVENTORY` | F01이 훅을 부르기 전에 실패하므로 **특가는 손도 안 댄다.** 카운터만 `INCR` |
...
F01에 의존하는 항목은 §3의 계약 표(C1~C6)와 아래 §13에 모았다. **12개 중 11개가 해소됐고, 남은 것은 V4 하나다.** V4에도 대체안이 있어 F02가 막히지는 않는다.

## 13. F01 재확인 목록

**2026-08-15 최종 갱신.** F01 세션이 PM 경유로 확장 지점(훅) 계약까지 전달하면서 **V3·V7·V8·V9·V10이 추가로 해소됐다.** 남은 미해소는 **V4 하나**이며, F01 스펙 승인 시 이것부터 대조한다.
...
| V4 | **[미해소·1순위]** 주문서 입력에 **특가 요청과 적용 단가**를 넘길 자리가 있는가 (계약 C6, 00 D5 조건) | 있다 | §3 C6, §6 흐름, D7, §8 응답 | 〈API〉 **남은 유일한 항목이자 가장 무겁다.** 없으면 ① 훅이 "이 예약이 특가 요청인지" 알 수 없어 **요청 범위 표시자라는 숨은 결합**을 써야 한다(§3 C6 대체안) ② F01 예약에는 정가가, 사용권에는 특가가 기록되어 **금액이 두 곳에서 갈린다** ③ 응답 `appliedPrice`의 의미가 "예약에 기록된 금액"에서 "사용권에 기록된 금액"으로 바뀌어 필드명을 `promotionPrice`로 갈아야 한다 ④ 제출 문서에 불일치를 명시해야 한다. **코드 변경은 작지만 외부 계약과 정직성 문서가 함께 움직인다** |
...
**12개 중 10개가 해소됐다.** 남은 것은 **V4(1순위)와 V10·V11**이며, 뒤 둘은 설정값과 응답 필드 하나다. F01 스펙 승인 시 V4부터 대조한다.

**해소 과정에서 설계가 두 번 뒤집혔다.** V3(생성 훅)이 D3를, V7(반납 훅)이 D6를 뒤집었다. 둘 다 **F01이 확장 지점을 열어주면서 "F01을 건드리지 않으려고 감수하던 대가"가 사라진 경우**다. 뒤집은 근거는 각 D 항목의 "왜 뒤집었나"에 남겼다.
