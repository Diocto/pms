...
F04의 특가 스파이크 시나리오가 이 숫자에 의존한다. **날짜만 미확정이다** — F01의 시드 데이터 계약이 확정되면 그 범위 안의 값으로 채운다 (§13 V12).

| 프로모션 | 대상 객실타입 | 날짜 | 특가 단가 | 한정 수량 | 판매 창 | 용도 |
|---|---|---|---|---|---|---|
| P1 열린 특가 | 시드 객실타입 중 **총 수량이 가장 큰 것** | `D+30` | 정가의 50% | **20** | 이미 열림 ~ `D+29` | **F04 S7 스파이크의 대상.** 특가 20실에 3초간 8000건 |
| P2 예정 특가 | 같은 객실타입, 다른 날짜 | `D+31` | 정가의 50% | 20 | `T+5분` ~ `D+30` | 오픈 순간 진입(S8) 재현용 |
| P3 닫힌 특가 | 다른 객실타입 | `D+32` | 정가의 60% | 10 | 이미 종료 | 판매 창 밖 거부(S6) 검증용 |

- `D` = F01 시드의 재고 시작 날짜. **`D+30`이 F01 재고 시드 범위 안에 반드시 들어야 한다.** 벗어나면 특가는 팔리는데 일반 재고 행이 없어 전건 실패하고, **응답이 409 재고부족이라 정상 동작처럼 보인다** (§13 V12)
- P1의 대상 객실타입은 **일반 재고가 특가 수량보다 충분히 커야 한다.** 20실 특가인데 그날 일반 재고가 20 미만이면 **F04 S7**이 특가 경합이 아니라 일반 재고 경합을 측정하게 된다. 권고: **그날 일반 재고 ≥ 100**
...
| **[확인]** | 판단은 섰다. 동의만 받으면 된다 | D4, D6, D7, D8, D10 |
...
**2026-08-15 갱신.** F01 세션이 PM 경유로 **재고 테이블 구조와 상태 전이 표 확정본**을 전달했다. 그에 해당하는 항목(V1·V2·V5·V6)은 아래에서 **[해소]**로 표시하고 본문에 반영을 끝냈다. 나머지는 여전히 추정이다.
...
| V12 | 시드 데이터의 날짜 범위·객실타입 id·수량·단가·테스트 `X-User-Id` 규약 | 90일 (F01 브리핑 참고값) | V202 시드, §9-3 | 〈설정〉 **시드 숫자 조정.** 특가 `stay_date`가 F01 재고 시드 범위 안에 들어야 한다. 범위 밖이면 일반 재고 행이 없어 전건 실패하는데 **응답이 409 재고부족이라 정상 동작처럼 보인다.** F04가 성공 0건을 받고 나서야 안다. **재확인 시 1순위** |

**설계 자체가 흔들리는 항목은 없다.** 무게 순으로 V4 → V12 → V11이고, 나머지는 어댑터 한 곳이나 문구 수정이다.

**V12는 PM 세션이 받아 처리 중이다.** 같은 문제가 F03(검색 날짜)·F04(시나리오 날짜)에도 있어, 개별 확인 대신 **F01 스펙에 "시드 데이터 계약" 절**을 만들어 날짜 범위·객실타입 id·수량·단가·초기 `remaining`·테스트 `X-User-Id` 규약을 숫자로 못박기로 했다. 그 계약이 확정되면 §9-3의 날짜를 채우고 V12를 닫는다.
