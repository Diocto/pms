...
F02의 해석기는 아래를 확인하고, 하나라도 어긋나면 빈 값을 돌려준다.

| 확인 | 어긋나면 |
|---|---|
| `reference`가 가리키는 특가가 존재하는가 | 빈 값 |
| 그 특가의 객실타입·날짜가 **인자로 받은 `roomTypeId`·`period`와 일치**하는가 | 빈 값. **사용자가 A 특가를 요청하고 B 객실을 예약하는 것을 막는다** |
| 판매 창 안인가 (주입한 `Clock` 기준) | 빈 값 |
| 1박 1실인가 (`period.nights() == 1`, `roomCount == 1`) | 빈 값. D4 제약을 여기서 강제한다 |
...
| 순서 | 대상 | 변화 | 트랜잭션 |
|---|---|---|---|
| 1 | Redis 멱등 키 | `SET NX`로 선점. 처리 중 표시 | 밖 (F02) |
| 2 | PromotionInventory (읽기) | 판매 창 검증. 창 밖이면 여기서 거부 | 밖 (F02) |
| 3 | Redis 잔여 카운터 | `DECR`. 결과가 음수면 즉시 `INCR`로 되돌리고 거부 | 밖 (F02) |
| 4 | — | F01 예약 생성 호출. 주문서에 `DiscountRef(PROMOTION, reference)`를 싣는다. **락 획득도 트랜잭션도 F01이 연다** | 밖 → 안 |
| 5 | RoomDailyInventory (해당 1일 행) | `remaining` 1 감소 | **단일 트랜잭션** (F01) |
| 6 | PromotionInventory (읽기) | **할인 해석.** 특가를 읽어 단가를 돌려준다. **쓰지 않는다** | 〃 (F02 해석기) |
| 7 | Reservation | `PENDING`으로 새로 생성 → `reservationId` 확정. **`price_per_night`에 특가 단가가 들어간다** | 〃 (F01) |
| 8 | PromotionInventory | `remaining` 1 감소 (조건부 UPDATE). **0건이면 예외를 던져 5~7까지 통째로 롤백시킨다** | 〃 (F02 생성 훅) |
| 9 | PromotionClaim | `USED` 상태로 새로 생성. `reservation_id`와 `applied_price` 기록 | 〃 (F02 생성 훅) |
| 10 | Redis 잔여 카운터 | 트랜잭션이 롤백됐으면 `INCR`로 되돌린다 | 커밋/롤백 후 (F02) |
| 11 | Redis 멱등 키 | 결과 저장 (TTL 24시간) | 밖 (F02) |

**6과 8이 같은 행을 읽고 쓴다.** 읽을 때 잔여가 있었는데 쓸 때 없어졌다면 8이 0건을 반환하고 전체가 롤백된다. **그래서 "할인가로 계산된 예약인데 특가는 안 깎인" 상태가 만들어질 수 없다.**
...
sequenceDiagram
    autonumber
    actor C as 클라이언트
    participant API as 특가 예약 API (F02)
    participant R as Redis
    participant F1 as 예약 생성 유스케이스 (F01)
    participant RS as 할인 해석기 (F02 구현)
    participant H as 생성 훅 (F02 구현)
    ...
    API->>DB: 특가 조회 · 판매 창 검증 (Clock 주입)
    alt 판매 창 밖
        API-->>C: 409 PROMOTION_NOT_OPEN
    end
    API->>R: 잔여 카운터 DECR
    alt 카운터 < 0 (매진)
        API->>R: INCR 되돌림
    ...
    API->>F1: 예약 생성 요청 (discounts=[PROMOTION:reference])
    F1->>R: 분산락 획득 (트랜잭션 밖)
...
| 상황 | 응답 | 부작용 처리 |
|---|---|---|
| 프로모션 없음 | 404 `RESOURCE_NOT_FOUND` | 없음. 카운터를 건드리기 전이다 |
| 판매 창 전·후 | 409 `PROMOTION_NOT_OPEN` | 없음. **카운터 게이트보다 앞에서 거른다** |
| Redis 카운터가 음수 (매진) | 409 `PROMOTION_SOLD_OUT` | 카운터 `INCR`로 즉시 되돌린다. **DB에 도달하지 않는다** |
| 특가 조건부 UPDATE 0건 (매진) | 409 `PROMOTION_SOLD_OUT` | 훅이 예외를 던져 **트랜잭션 전체 롤백.** 일반 재고와 예약도 되돌아간다. 카운터 `INCR` |
| 일반 재고 부족 (F01) | 409 `INSUFFICIENT_INVENTORY` | F01이 해석기·훅을 부르기 전에 실패하므로 **특가는 손도 안 댄다.** 카운터만 `INCR` |
| 해석기가 빈 값 반환 (F02 검증 후 판매 창이 닫힌 경우 등) | 400 `INVALID_REQUEST` (F01이 낸다) | 트랜잭션 롤백. 카운터 `INCR`. **좁은 경합 구간에서만 나온다** — F02가 이미 판매 창을 확인한 뒤 F01에 들어가기까지의 틈이다. 정가로 조용히 넘어가지 않는 것이 핵심이라 이 응답을 그대로 노출한다 |
| 멱등 키 재요청 (완료된 건) | 200, 저장된 결과 그대로 | 없음. 재처리하지 않는다 |
| 멱등 키 재요청 (처리 중) | 409 `REQUEST_IN_PROGRESS` | 없음 |
| 커밋 직후 프로세스 죽음 | 응답 없음 (클라이언트 타임아웃) | **데이터는 정합하다.** 예약과 사용권이 함께 커밋됐다. 카운터 `INCR`이 안 됐을 수 있으나 그건 과소 판매 방향이고 재동기화(UC-7-D)가 회복한다. 멱등 키에 결과가 안 남았으면 클라이언트 재시도가 409 `DUPLICATE_REQUEST`로 막힌다 |
...
**락이 없어도 정합성이 지켜지는 근거.** 이건 추측이 아니라 증명 대상이다. F01이 K2에서, F02가 S2에서 같은 명제를 증명한다 — **1차 방어를 통째로 끄고 재고 N에 N보다 많은 스레드를 붙여도 정확히 N건만 성공한다.** 분산락과 카운터 게이트는 DB 앞에서 경합을 줄이는 1차 방어일 뿐이고, **정합성은 조건부 UPDATE와 CHECK가 책임진다.**
...
- **F01로 내려보내는 멱등 키:** `promo:{클라이언트 키}`로 접두사를 붙여 넘긴다. 같은 사용자가 일반 예약과 특가 예약에 우연히 같은 키를 써도 서로를 중복으로 오인하지 않게 하려는 것이다. 재시도해도 같은 값이 나오므로 F01 쪽 멱등도 그대로 성립한다
- **두 겹의 멱등이 겹치는 순서:** F02의 선점이 먼저다. 재시도는 F02 단계에서 저장된 결과를 받고 되돌아가므로 F01까지 내려가지 않는다. F01의 멱등은 F02가 뚫렸을 때를 위한 것이고, 그마저 뚫려도 `uk_claim_reservation`이 한 예약에 사용권 두 장을 막는다
...
```json
{
  ...
  "roomTypeId": 3,
  "stayDate": "2026-09-12",
  "appliedPrice": 75000.00,
  "reservationStatus": "PENDING"
}
```

`appliedPrice`는 **실제로 청구된 1박 단가**다. `reservation.price_per_night`과 같은 값이며(D7), 정가는 `room_type.base_price`로 언제든 조회할 수 있으므로 응답에 따로 싣지 않는다.
...
**판매 창 밖 트래픽은 DB까지 온다.** 카운터 게이트는 잔여만 보고 시각은 보지 않는다. 오픈 대기 트래픽은 1차에서 걸리지 않고 **카운터를 건드리기 전의 판매 창 검증**에서 거부된다(특가 행 조회 1회). 이 한계를 알고 부하테스트에서 측정한다. 오픈 대기 트래픽은 잔여 조회 API(UC-7-B)로 유도하는 것이 설계 의도다.
