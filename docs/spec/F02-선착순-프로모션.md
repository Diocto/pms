...
| T18d | 해석기가 빈 값을 돌려주는 네 경우 전수 — 없음 / 객실타입·날짜 불일치 / 판매 창 밖 / 1박1실 아님 | fail-closed |
...
- **대안이었던 것:**
  ...
  | 대안 | 얻는 것 | 잃는 것 |
  |---|---|---|
  | **A. 지금 결정 — 게이트 없이 간다** | 계약 추가 없음. 코드가 가장 단순 | 매진 트래픽이 DB까지 온다 |
  | **B. F01에 "트랜잭션 전 훅"을 하나 더 요청** | 게이트가 제자리를 찾는다. 기존 훅 둘과 형태가 같고, F01은 이미 트랜잭션 전 단계(락 획득)를 갖고 있다 | 계약이 한 번 더 늘고 F01 작업이 는다. 일정이 촉박하다 |
  | **C. F02가 `POST /api/reservations`에 서블릿 필터를 건다** | F01 계약 무변경 | 본문을 필터에서 파싱해야 해서 취약하고, F01의 웹 계층에 F02가 끼어드는 형태라 소유 경계가 흐려진다. **기각** |
- **확인 질문:** **A로 갈까요, B로 갈까요?** 저는 일정을 고려하면 **A**를 권합니다 — 정확성은 어느 쪽이든 같고, 잃는 것은 스파이크 때의 처리량입니다. 다만 **F04의 특가 스파이크 시나리오가 측정하려는 것이 정확히 그 처리량**이라, "매진 트래픽을 DB 앞에서 자른다"는 그림을 부하테스트 리포트에서 보여주고 싶다면 B가 필요합니다. 이건 제가 혼자 정할 문제가 아니라고 봅니다.
