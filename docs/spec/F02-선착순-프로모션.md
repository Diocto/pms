...
F02는 F01 위에 얹힌다. **F01이 확장 지점 두 개를 인터페이스로 정의했고, F02가 그 구현체를 제공한다.** F01은 promotion이라는 것이 존재하는지도 모른다. 의존은 **F02 → F01 한 방향**이다.
...
| C2 | `ReservationReleaseHook` — 상태 전이가 실제로 일어난 1건에 대해 **정확히 한 번** 호출. `NO_SHOW`는 호출하지 않는다 | **확정** |
...
| 2 | 분산락 | 재고 날짜 락 획득 | 밖 | F01 |
| 3 | RoomDailyInventory (해당 1일 행) | `remaining` 1 감소 | **단일 트랜잭션** | F01 |
| 4 | PromotionInventory (읽기) | **할인 해석.** 특가를 읽어 단가를 돌려준다. **쓰지 않는다** | 〃 | **F02 해석기** |
| 5 | Reservation | `PENDING`으로 새로 생성 → `reservationId` 확정. **`price_per_night`에 특가 단가가 들어간다** | 〃 | F01 |
| 6 | PromotionInventory | `remaining` 1 감소 (조건부 UPDATE). **0건이면 예외를 던져 3~5까지 통째로 롤백시킨다** | 〃 | **F02 생성 훅** |
| 7 | PromotionClaim | `USED` 상태로 새로 생성. `reservation_id`와 `applied_price` 기록 | 〃 | **F02 생성 훅** |
| 8 | Redis 잔여 카운터 | 커밋됐으면 `DECR`, 롤백됐으면 건드리지 않는다 | 커밋 후 | **F02** |
| 9 | Redis 멱등 키 | 결과 저장 | 밖 | F01 |

**F02가 손대는 것은 4·6·7·8 넷뿐이다.** 요청을 받는 것도, 락을 잡는 것도, 트랜잭션을 여는 것도, 멱등을 처리하는 것도 전부 F01이다. **F02는 F01의 예약 생성 흐름 안에 얹힌 세 조각**(해석기·생성 훅·커밋 후 카운터 갱신)이다.

**4와 6이 같은 행을 읽고 쓴다.** 읽을 때 잔여가 있었는데 쓸 때 없어졌다면 6이 0건을 반환하고 전체가 롤백된다. **그래서 "할인가로 계산된 예약인데 특가는 안 깎인" 상태가 만들어질 수 없다.**
...
**어떤 이벤트에 훅이 오는가.** F01이 `NO_SHOW` 전이에서는 훅을 부르지 않는다. 그래도 F02는 `event`를 보고 **자기 쪽에서도 반납 대상인지 판단한다.** 판단 근거를 F01의 호출 여부에만 두면, F01의 정책이 바뀌었을 때 F02가 조용히 따라가 버린다.
...
**`NO_SHOW`를 반납하지 않는 것은 F01의 일반 재고 정책과 대칭이다.** 노쇼는 요금을 받고 방을 잡아둔 것이라 F01도 일반 재고를 복원하지 않는다(F01 D15). 특가만 반납하면 "방은 안 돌아왔는데 특가는 돌아온" 어긋난 상태가 된다. **이 대칭을 깨려면 정책 변경이므로 관리자 판단이 필요하다.**
...
| 계층 | 수단 | 무엇을 막는가 | 뚫리면? |
|---|---|---|---|
| 1차 | **없다.** F02가 트랜잭션 전에 실행될 자리가 없다 (D5) | — | — |
...
**1차 방어가 비어 있다는 것을 감추지 않는다 (D5).** 초안에서는 Redis 잔여 카운터가 1차였다. 특가 예약 전용 엔드포인트가 없어지면서 **트랜잭션이 열리기 전에 F02 코드가 실행될 자리가 사라졌고**, 게이트는 그 자리에서만 의미가 있다. 되살릴지 여부는 D5의 확인 질문으로 올렸다.

**그래서 F02의 정확성은 2차와 3차만으로 지켜진다.** 이건 약점이 아니라 원래 명제다 — `coding-rules.md`의 다층 방어 절이 요구하는 것은 "1차가 뚫려도 2차가 막는다"이고, F02는 **1차가 아예 없는 상태로도 불변식이 지켜지는 것**을 증명한다. 실제로 초안의 S2(게이트를 끄고 S1을 반복)가 검증하려던 명제가 지금은 **기본 동작**이 됐다.

특가 재고는 **행이 하나**라서 락 순서 문제가 없고, 조건부 UPDATE 한 줄이면 원자성이 완결된다. 락으로 얻을 것이 없다.

**잃은 것은 처리량이다.** 매진 이후 요청도 락을 잡고 트랜잭션을 열고 조건부 UPDATE 0건을 받고 롤백한다. **F04의 스파이크 시나리오가 측정할 것이 정확히 이 비용이다.**
...
| 동시성 | 아래 표 |
| 통합 (F01 연동) | 예약 취소가 실제로 특가를 되돌리는지, `NO_SHOW`가 되돌리지 않는지, **매진 시 예약까지 롤백되는지** |
...
| S2 | **Redis를 통째로 끈 상태로 S1 반복** | 200 | 10 | 충분(100) | S1과 동일. **정확성이 Redis에 전혀 의존하지 않음을 증명한다.** 이 테스트가 통과하지 못하면 F02는 완성이 아니다 |
...
| S2 차이 | **Redis를 내린 채로** 그대로 반복한다. 기대 결과가 S1과 완전히 같아야 한다. 다르면 정확성이 Redis에 의존한다는 뜻이고, 그건 Redis 장애 시 초과 판매가 난다는 뜻이다. **F02는 카운터를 조회용으로만 쓰므로 이 테스트는 설계상 통과해야 정상이다** |
...
| T26 | **반납 훅**이 `CANCELLED`·`EXPIRED`에서 재고를 되돌리고, **`NO_SHOW`에서는 되돌리지 않음** | 반납 대상 판정 |
...
**매진·판매창밖 트래픽이 전부 DB까지 온다.** F02가 트랜잭션 전에 실행될 자리가 없어서다(D5). 판매 창 밖 요청은 해석기에서, 매진 요청은 훅에서 거부되는데 **둘 다 이미 락을 잡고 트랜잭션을 연 뒤**다. 이 한계를 알고 부하테스트에서 측정한다. **오픈 대기 트래픽을 잔여 조회 API(UC-7-B)로 유도하는 것이 지금 남은 유일한 완충 장치**이므로, 조회 API는 반드시 Redis에서 응답해야 한다.
...
### D5. [필수] 특가에는 락을 두지 않는다. 잔여 카운터는 조회 전용으로 내린다

> **이 항목은 2026-08-15에 축소됐다.** 초안에서는 Redis 잔여 카운터가 **1차 방어(매진 트래픽을 DB 앞에서 잘라내는 게이트)**였다. **특가 예약 전용 엔드포인트가 없어지면서 그 게이트를 놓을 자리가 사라졌다.** 아래 "게이트를 잃은 경위"에 적었고, 되살릴지 여부는 확인 질문으로 올린다.

- **결정:** ① 특가 재고에 분산락을 두지 않는다. 조건부 UPDATE 한 줄로 원자성이 완결된다. ② Redis 잔여 카운터는 유지하되 **잔여 조회 API(UC-7-B) 전용**으로 쓴다. 요청을 막는 게이트로는 쓰지 않는다.
- **이유(락을 안 두는 것):** 락은 경합을 없애는 게 아니라 줄을 세운다. 잔여 10개에 500요청이면 매진 후 490개가 락을 순서대로 기다렸다가 DB를 한 번씩 두드린다. 게다가 특가 재고는 **행이 하나**라 락 순서 문제가 없고 조건부 UPDATE로 원자성이 이미 보장된다. 락이 보태는 것이 없다. 그리고 훅은 트랜잭션 안에서 불리므로 **거기서 락을 잡는 것은 금지**돼 있다(§7).
- **게이트를 잃은 경위:** 게이트는 **트랜잭션이 열리기 전에** 실행돼야 의미가 있다. 매진 요청이 DB 커넥션을 잡기 전에 잘라내는 것이 목적이기 때문이다. 초안에서는 F02가 자기 엔드포인트를 갖고 있어서 거기가 자리였다. **지금은 요청을 F01이 받고, F02는 트랜잭션 안에서만 불린다**(해석기·훅). 즉 F02가 코드를 실행할 수 있는 가장 이른 시점이 이미 락을 잡고 트랜잭션을 연 뒤다. 거기서 게이트를 돌리면 **막으려던 비용을 이미 다 치른 뒤**라 의미가 없고, 트랜잭션 안 원격 호출 금지 규칙도 어긴다.
- **트레이드오프:** **매진 이후의 스파이크 트래픽이 전부 DB까지 온다.** 잔여 20개짜리 특가에 8000건이 몰리면 7980건이 락을 잡고 트랜잭션을 열고 조건부 UPDATE 0건을 받고 롤백한다. **정확성은 그대로다**(2차·3차가 막는다) — 잃는 것은 처리량과, 그 순간 일반 예약이 쓸 커넥션이다. §1이 내세운 "한 점에 몰리는 경합은 방어 수단이 다르다"는 서사도 그만큼 약해진다.
- **대안이었던 것:**

  | 대안 | 얻는 것 | 잃는 것 |
  |---|---|---|
  | **A. 게이트 없이 간다** | 계약 추가 없음. 코드가 가장 단순 | 매진 트래픽이 DB까지 온다 |
  | **B. F01에 "트랜잭션 전 훅"을 하나 더 요청** | 게이트가 제자리를 찾는다. 기존 훅 둘과 형태가 같고, F01은 이미 트랜잭션 전 단계(락 획득)를 갖고 있다 | 계약이 한 번 더 늘고 F01 작업이 는다 |
  | **C. F02가 `POST /api/reservations`에 서블릿 필터를 건다** | F01 계약 무변경 | 본문을 필터에서 파싱해야 해서 취약하고, F01의 웹 계층에 F02가 끼어드는 형태라 소유 경계가 흐려진다. **기각** |

- **B를 지지하는 근거 둘 (PM 세션이 덧붙인 것):**
  1. **다층 방어 서사의 일관성.** 일반 재고는 3층인데 특가만 2층이 된다. 제출 문서에서 "3층 방어를 두었다"고 쓸 때 특가는 예외라는 각주를 또 달아야 한다.
  2. **F04가 이것을 측정할 수 있다.** 게이트를 껐다 켜는 스위치를 두면 **"매진 트래픽을 DB 앞에서 자르면 처리량이 얼마나 달라지는가"**가 수치로 나온다. 이 프로젝트가 파는 것이 정확히 그런 상대 비교다.
- **확인 질문:** **A와 B 중 어느 쪽입니까?** 비용을 아는 쪽이 F01이라 PM 세션이 "트랜잭션 전 훅 하나가 얼마나 비싼가"로 좁혀 F01에 물어둔 상태입니다. **거의 공짜라면 B를, 그렇지 않으면 A를 권합니다.** 정확성은 어느 쪽이든 같고 차이는 스파이크 때의 처리량과 리포트에 담을 수 있는 그림입니다.

### D5 부록. B로 결정될 경우의 게이트 설계 (미리 확정)

결정을 기다리는 동안 설계를 못박아 둔다. **게이트를 붙이게 되면 반드시 fail-open이다.**
...
| V7 | **[해소]** 취소·만료를 F02가 아는 방법 | 확정본 수령: `ReservationReleaseHook`. 상태 전이 1건 성공 시 같은 트랜잭션에서 **정확히 한 번** 호출. `NO_SHOW`는 호출 안 함 | §6 UC-7-C, D6 | — 반영 완료. **D6를 폴링에서 훅으로 뒤집었다.** 예약 상태를 따로 조회할 필요가 없어졌다 |
