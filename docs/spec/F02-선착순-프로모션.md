...
- **Alembic 리비전 대역:** `201` ~ `299` (`--rev-id`로 지정한다. 옛 Flyway `V201~V299`와 같은 대역)
- **수정이 필요한 공유 파일** (둘 다 parallel-work의 공유 파일 절차 — 해당 변경만 담은 작은 커밋을 main에 먼저 — 를 따른다 → **D12**)
  - `app/common/errors.py`에 예외·에러 코드 2개 추가 (`PROMOTION_NOT_OPEN`, `PROMOTION_SOLD_OUT`)
  - `app/common/config.py`에 게이트 스위치 1개 추가 (기본 켬. F01의 락 스위치와 같은 성격 → §7, D5)

> **⚠ `parallel-work.md`의 경로 표현이 아직 Java 판이다** (2026-08-15 확인. Flyway 대역 표, `build.gradle`, `application.yml`, `com.pms.common.**`). PM 세션이 갱신 예정이라고 알려왔다. **대역을 나눈다는 규약 자체는 그대로**이므로 위 선언은 새 경로로 미리 적어둔다. 갱신본이 나오면 대조한다.
...
├── adapter/
│   ├── web/              GET /api/promotions/{room_type_id}/{stay_date}
│   ├── persistence/      조건부 UPDATE 쿼리, F01 확장 지점 구현체
│   └── cache/            Redis 잔여 카운터
└── container.py          이 컨텍스트의 프로바이더 묶음
```

**F01 확장 지점의 구현체는 `adapter/persistence/`에 둔다.** 안쪽이 아니라 바깥이다 — 이 구현체들은 F01의 `Protocol`을 만족시키는 어댑터이고, 세션을 받아 SQL을 실행하는 것이 하는 일의 전부다. **`app/promotion/domain/`은 F01을 import하지 않는다.**
...
> - **구현체는 `session_factory`를 주입받지 않는다.** 받을 수 없으면 열 수 없다. **F02 컨테이너에서 훅 구현체에 `session_factory`를 주입하지 않는다**
> - **정적으로도 본다.** `app/promotion/adapter/persistence/` 안에서 `sessionmaker`·`create_engine`·`session_factory`를 import하지 않는다. import가 없으면 만들 수단이 없다
...
F01의 예약 생성 유스케이스가 훅들을 `list[...]`로 주입받아 전부 호출한다. **구현이 0개면 빈 리스트라 아무 일도 일어나지 않는다.** 그래서 F02 병합 전에도 F01은 그대로 돌고, F02가 잘려나가도(00 D4 절단 3순위) F01은 멀쩡하다. F02는 컨테이너에 프로바이더를 등록하면 자동으로 들어온다.

> **⚠ 이 보장의 위치가 내려갔다.** Java에서는 `List<Hook>` 주입이라 **타입 시스템이 강제**했다. 지금은 **컨테이너 조립 설정**에 있고, **프로바이더 등록 실수는 컴파일러가 안 잡아준다.**
>
> `clean-architecture.md`가 이것을 테스트로 고정하라고 요구한다 — **"확장 feature의 프로바이더를 등록하지 않고 코어 유스케이스가 그대로 동작한다."** F02는 절단 후보 3순위라(00 D4) 이 성질이 실제로 쓰일 수 있다. **T35**로 넣었다.
...
> | 정적 확인 | `app/promotion/adapter/persistence/`가 `sessionmaker`·`create_engine`·`session_factory`를 import하지 않는다 |
>
...
**여러 개를 잡게 되면 정렬은 포트 안에서 한다.** 지금 설계로는 특가 락이 **하나뿐**이라 해당 없지만, 늘어날 때를 위해 방식을 미리 고정한다. F01이 D10에서 확정한 형태가 이것이다 — `LockPort.acquire_all(keys: list[str], ttl_seconds: int)`가 **목록을 받아 자기가 정렬한 뒤** 잡고, 컨텍스트 매니저로 돌려줘 예외가 나도 반드시 풀리게 한다.
...
> **⚠ 미결 — API 표기 규칙이 아직 안 정해졌다 (스택 전환으로 새로 생긴 항목).**
>
> 아래 예시는 Java 판에서 쓰던 **camelCase**(`roomTypeId`, `pricePerNight`, 경로 `{roomTypeId}`)를 그대로 두었다. FastAPI 관례는 **snake_case**(`room_type_id`)이고, `coding-rules.md`도 파이썬 식별자는 snake_case로 못박았다.
>
> **F02가 임의로 정하지 않는다.** 예약 생성 API의 형식은 F01 소유이고, **경로·JSON 필드 표기는 F01·F02·F03이 반드시 같아야 한다.** 한쪽만 바꾸면 클라이언트가 두 규칙을 동시에 다루게 된다.
>
> **F02의 입장:** snake_case로 통일하는 쪽이 낫다. 그렇지 않으면 웹 계층에서 이름을 바꾸는 번역이 한 겹 생기고, **그 번역은 §7의 설정 키 규칙("조작자가 치는 그대로")과 같은 이유로 재현성을 깎는다** — 부하테스트 스크립트와 문서의 필드명이 코드와 달라진다.
>
> **PM 경유로 F01·F03과 합의되면 이 절의 예시를 일괄 교체한다.** 그 전까지는 표기만 미정이고 **엔드포인트 구성·필드 구성·에러 코드는 확정**이다.
...
| **T22b** | **생성 훅이 성공한 뒤 F01이 실패하면 특가 차감과 사용권 행도 사라진다** | **세션 분리를 잡는 유일한 테스트.** 이게 없으면 §7의 함정을 잡는 수단이 없다. F01 T79b와 짝 |
...
| **T35** | **F02 프로바이더를 등록하지 않고 F01 예약 생성이 그대로 동작함** | **절단 3순위(00 D4) 대비.** 보장이 타입 시스템에서 조립 설정으로 내려갔으므로 컴파일러가 안 잡아준다 (`clean-architecture.md`) |
...
| **T37** | `app/promotion/adapter/persistence/`가 `sessionmaker`·`create_engine`·`session_factory`를 **import하지 않음** (정적 확인) | 세션 분리 함정의 정적 방어선. **import가 없으면 만들 수단이 없다** |
...
**T35~T37은 전부 "조용한 실패"를 잡는다.** 셋 다 없어도 다른 테스트는 전부 초록이고, 셋이 막는 것은 전부 **아무것도 안 봤는데 통과로 보이는** 상태다. Java 판에서는 컴파일러와 타입 시스템이 하던 몫이 여기서는 테스트로 내려왔다.
...
**F02를 통째로 들어내도 F01이 돌아야 한다** (00 D4: F02는 절단 후보 3순위). 훅이 리스트 프로바이더라 **F02 프로바이더가 등록되지 않으면 빈 리스트가 되어 아무 일도 일어나지 않는다.** 이 성질을 깨는 변경을 하지 않는다. 의존 방향은 항상 F02 → F01 한 방향이다.
...
> **⚠ 훅 안에서 하면 안 되는 것**
>
> - **세션을 새로 여는 것.** 이 스택에서 **가장 조용한 함정**이다. 받은 `session`만 쓴다. §7 참조. **`session_factory`를 주입받지 않고, `sessionmaker`·`create_engine`을 import하지 않는다**
...
**도메인은 `sqlmodel`·`sqlalchemy`와 표준 라이브러리만 의존한다.** `fastapi`·`redis`·`dependency_injector`가 `app/promotion/domain/`에 들어오면 규칙 위반이다. **게이트(Redis)는 `adapter/cache/`에, 훅 구현체는 `adapter/persistence/`에 둔다.**
...
**등급 안내.** 17개가 같은 무게가 아닙니다. 등급으로 나눴으니 위에서부터 보시면 됩니다.

| 등급 | 뜻 | 해당 항목 |
|---|---|---|
| **[필수]** | 뒤집히면 스펙을 다시 써야 한다. 여기에 시간을 쓰시면 된다 | D1, D2, D3, D5, **D16** |
| **[확인]** | 판단은 섰다. 동의만 받으면 된다 | D4, D6, D8, D10, D14, **D17** |
...
>
> **D17(API 표기)은 F02가 정할 수 없어 제안으로 올립니다.** 세 feature가 같아야 하는 항목입니다.
...
- **결정:** F02 컨테이너에서 **확장 지점 구현체(사전 검사·해석기·생성 훅·반납 훅)에는 `session_factory`를 주입하지 않는다.** 받은 `session`만 쓴다. 더해서 ① 구현 모듈이 `sessionmaker`·`create_engine`·`session_factory`를 import하지 않는지 정적으로 확인하고(**T37**), ② **훅이 성공한 뒤 F01이 실패하는** 경로를 테스트한다(**T22b**).
...
- **확인 질문:** 훅 구현체에서 세션을 열 수단 자체를 없애고(주입 안 함 + import 정적 확인), **T22b로 "훅 성공 후 호출부 실패" 경로를 반드시 테스트하는** 것으로 가도 괜찮습니까?

### D17. [확인] API 표기(경로·JSON 필드)를 snake_case로 통일할 것을 제안한다

> **스택 전환으로 새로 생긴 항목이다.** F02 단독으로 정할 수 없어 제안으로 올린다.

- **결정:** **정하지 않고 올린다.** F02의 입장은 snake_case 통일이다. 다만 예약 생성 API는 F01 소유이고 표기는 **F01·F02·F03이 반드시 같아야** 하므로, 관리자 판단과 PM 조율이 필요하다.
- **이유:** `coding-rules.md`가 파이썬 식별자를 snake_case로 못박았다. API만 camelCase로 두면 **웹 계층에서 이름을 바꾸는 번역이 한 겹 생긴다.** 그 번역은 §7의 설정 키 규칙("조작자가 치는 그대로")과 같은 이유로 재현성을 깎는다 — **부하테스트 스크립트와 문서의 필드명이 코드와 달라지고, 리포트를 보고 재현하려는 사람이 그대로 붙여넣을 수 없다.**
- **트레이드오프:** 지금 세 스펙의 API 예시를 전부 고쳐야 한다. 그리고 **표기를 안 고치는 쪽도 완전히 틀린 선택은 아니다** — camelCase JSON은 웹 API에서 흔하고, 번역 계층을 한 곳(스키마 정의)에 몰아두면 실제 비용은 크지 않다.
- **대안이었던 것:** camelCase 유지(번역 한 겹을 받아들인다), 경로만 snake_case로 하고 본문은 camelCase(**가장 나쁘다** — 한 API 안에서 두 규칙이 섞인다).
- **확인 질문:** API 경로와 JSON 필드를 snake_case로 통일해도 괜찮습니까? 아니면 camelCase를 유지할까요? **어느 쪽이든 세 feature가 같아야 하므로 PM이 일괄 전파해야 합니다.**

## 12. 열린 질문

**하나 있다 — API 표기 규칙이다 (D17).** 스택 전환으로 새로 생겼고, **F02가 정할 수 없는 종류**다. 경로와 JSON 필드를 snake_case로 갈지 camelCase로 둘지는 F01·F02·F03이 반드시 같아야 하므로 관리자 판단과 PM 전파가 필요하다. **엔드포인트 구성·필드 구성·에러 코드는 확정이고, 표기만 미정이다.**

F01에 의존하는 계약 항목(§3 C1~C9, §13)은 **13개가 전부 해소됐다.** 스택 전환 뒤에도 계약 내용은 하나도 바뀌지 않았고, **세션 인자가 붙으면서 C9가 하나 늘었다.**

**그리고 스택 전환으로 새로 생긴 것 둘을 D 항목으로 올렸다.**

| 항목 | 왜 새로 생겼나 |
|---|---|
| **D16** [필수] 훅 구현체에 `session_factory` 주입 금지 | Spring은 트랜잭션이 자동 공유됐지만 SQLAlchemy는 아니다. **D3의 명제가 구현 실수로 조용히 깨질 수 있다** |
| **D17** [확인] API 표기 통일 | 파이썬 관례가 snake_case인데 지금 예시가 camelCase다. 세 feature가 같아야 한다 |
