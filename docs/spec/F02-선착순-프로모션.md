...
- **수정이 필요한 공유 파일** (둘 다 parallel-work의 공유 파일 절차 — 해당 변경만 담은 작은 커밋을 main에 먼저 — 를 따른다 → **D12**)
  ...
  - `app/common/config_report.py`의 `ConfigContributor` 구현 1개 등록 (파일 자체는 F01이 먼저 만든다 → §7 설정 노출)
...
**선이 재는 대상이 확정됐다** (2026-08-15, F01 D23). 설정 노출 기여자(`ConfigContributor`, §7)가 이 선에 걸리는지가 실제로 문제 됐고, F01이 **안 걸린다**로 판단했다 — 선의 걱정은 "순서가 계약이 된다"와 "구현자가 하나뿐인 추상화" 둘인데, 설정 기여자는 요청 경로 밖 읽기 전용이라 순서가 없고 구현자가 F02·F03 둘이다. **예약 생성 흐름의 훅은 여전히 넷이 상한이다.**
...
**셋을 함께 싣는다.** F02 기여자가 실물 계약(`app/common/config_report.py`, 아래 확정 블록)에 맞춰 내놓는 모양은 이렇다.
...
# 카운터 둘(rejected_by_gate, rejected_by_conditional_update)의 자리는 아래 참조
```
...
> **⚠ 카운터 둘의 자리는 실물 계약에 아직 없다 (2026-08-15 실물 대조에서 확인).** `ConfigReport`의 그릇은 `load_test`·`implementations` 둘뿐인데, **카운터는 어느 쪽 규칙에도 맞지 않는다** — `load_test` 키는 "셸에 치는 환경변수 이름 그대로"인데 카운터는 환경변수가 아니고, `implementations` 값은 클래스 이름 문자열이다. 억지로 밀어 넣으면 그릇의 규칙이 거짓이 된다.
>
> **F01에 `counters: dict[str, int] = {}` 필드 추가를 제안한다** (PM 경유, 기본값이 빈 dict라 F01·F03 기여자는 손대지 않아도 된다). 성격이 다른 값이라는 것이 근거다 — 설정·구현은 **실행 전** 확인이고 카운터는 **실행 후** 판정이다(아래 표). 계약 소유가 F01이라 F02가 정하지 않는다. **거절되면 대안은 게이트 카운터를 별도 운영 응답에 싣는 것**인데, 그건 "두 번째 노출 경로"가 아니다 — 그 금지는 같은 **설정**을 두 곳에서 읽는 것에 대한 규칙이고 카운터는 설정이 아니다. 다만 F04가 읽을 곳이 둘로 늘어나므로 계약 확장 쪽이 낫다.
...
> **✅ 탑재 방식이 확정됐고 실물이 나와 있다 (2026-08-15, F01 D26. `origin/worktree-F01:app/common/config_report.py`에서 대조 완료).** 컨텍스트별 기여자를 리스트 프로바이더로 모으는 1안이다.
>
> ```python
...
> ```
>
> - **이름이 `ConfigContributor`다.** 이 문서의 이전 판본은 PM 전달 초안의 `ConfigContributor`로 적었는데, 실물 대조에서 확정 이름으로 바로잡았다
...
> - **F02는 자기 컨테이너에 `ConfigContributor` 구현 하나를 등록한다.** F01 라우트가 리스트 프로바이더로 모아 `merge_reports`로 합친다. **구현 0개면 자연히 안 나온다** — F02가 잘려도 F01이 그대로 돈다 (T35와 같은 성질)
...
**설정 노출 엔드포인트는 F02가 만들지 않는다.** F01이 만들고, **F02는 `app/common/config_report.py`의 `ConfigContributor` 구현 하나를 자기 컨테이너에 등록한다** (탑재 방식 확정 — §7 설정 노출 절). 싣는 내용은 스위치 값 + 게이트 구현 클래스 이름 + 카운터 둘이고, 값의 출처는 **게이트가 실제로 쓰는 것과 같은 프로바이더**여야 한다(§7, T18a-6). **F02가 임의로 두 번째 노출 경로를 만들면 그 순간 "두 곳에서 따로 읽는" 상태가 된다.**
