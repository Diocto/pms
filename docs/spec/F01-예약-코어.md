...
```
1. 멱등성 키 선점 (Redis SET NX). 실패 시:
   - 결과가 있으면 그 결과를 그대로 응답 (재시도로 판단)
   - 처리 중이면 409 REQUEST_IN_PROGRESS
2. 분산락 획득: lock:inventory:{roomTypeId} (D2)
3. 트랜잭션 시작
4. 재고 차감: 날짜 오름차순으로 조건부 UPDATE
   UPDATE room_daily_inventory SET remaining = remaining - :n
    WHERE room_type_id = :rt AND stay_date = :d AND remaining >= :n
   → 갱신 행 수가 날짜 수와 다르면 재고 부족. 전체 롤백
5. Reservation INSERT (PENDING)
6. 커밋
7. 락 해제
8. 멱등성 키에 결과 저장, 응답
```
...
```
1. 예약 조회, 본인 확인. PENDING이 아니면 멱등 규칙 적용
2. 모의 결제 호출 (트랜잭션 밖. coding-rules의 원격 호출 규칙)
3-a. 성공: UPDATE reservation SET status='CONFIRMED' WHERE id=:id AND status='PENDING'
     → 0건이면 그 사이 만료·취소된 것. 결제 취소(모의)를 부르고 현재 상태 기준으로 응답
3-b. 실패: 같은 방식으로 CANCELLED 전이 + 재고 복원 (한 트랜잭션)
```

2와 3 사이의 틈이 이 유스케이스의 경합 지점이다. 결제는 성공했는데 그 사이 EXPIRE가 이긴 경우, 조건부 UPDATE 0건으로 감지하고 보상(모의 결제 취소)한다. 이 순서를 스펙으로 못박는다.
