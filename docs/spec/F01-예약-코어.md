...
**그림 해설.** 이 모델의 등장인물은 다섯이다.

- **Reservation(예약서)**: 누가(userId), 어떤 객실타입을(roomTypeId), 언제부터 언제까지(stayPeriod), 몇 실(roomCount), 얼마에(totalPrice) 예약했는지와 지금 어떤 상태인지(status)를 담는다. 시스템의 중심이고, 상태를 바꾸는 유일한 방법은 이벤트를 던지는 `handle()`이다.
- **StayPeriod(투숙 기간)**: 체크인·체크아웃 날짜 묶음이다. 낱개 날짜 두 개로 두지 않고 묶은 이유는 "체크아웃은 체크인보다 뒤"라는 규칙을 생성 시점에 강제하고, "이 기간이 차지하는 날짜 목록"(occupiedDates) 같은 계산을 한곳에 두기 위해서다. 다이아몬드 실선(`*--`)은 StayPeriod가 예약서에 소속된 부품이라는 뜻이다.
- **ReservationStateMachine(상태 전이표)**: "지금 상태에서 이 이벤트가 오면 다음 상태는 무엇인가"를 담은 표다. 예약서는 스스로 판단하지 않고 이 표에 물어본다.
- **RoomDailyInventory(일별 재고)**: 객실타입 하나의 하루치 잔여 수량이다. "8월 20일 디럭스 잔여 3" 같은 행 하나가 이 객체 하나이고, 동시성 제어가 벌어지는 무대다.
- **RoomType / Hotel(참조 데이터)**: 시드로 미리 넣어두는 읽기 전용 정보다.

점선 화살표(`..>`)는 전부 **ID로만 참조**한다는 표시다. 예약서가 재고나 객실타입 객체를 직접 들고 다니면 수정 범위와 잠금 범위가 흐려지므로, 번호만 기억하고 필요할 때 각자 불러온다.
...
**그림 해설.** 테이블 넷의 역할은 이렇다.

- **hotel → room_type**: 호텔 하나가 여러 객실타입을 가진다(1:N). 둘 다 시드 데이터라서 이 feature에서 쓰기는 없다.
- **room_type → room_daily_inventory**: 객실타입 하나가 날짜별 재고 행을 여러 개 가진다. 예를 들어 디럭스의 90일치 재고면 행 90개다. `(room_type_id, stay_date)` 유니크 제약은 "같은 타입·같은 날짜의 재고 행은 하나뿐"을 보장하고, 덕분에 조건부 UPDATE가 겨냥할 행이 항상 정확히 하나로 정해진다. `remaining`의 CHECK 제약은 어떤 코드 경로로도 잔여가 음수로 저장될 수 없게 막는 최후 방어선이다.
- **room_type → reservation**: 예약이 어느 객실타입인지 번호로 가리킨다. 외래키 제약을 걸지 않고 ID만 저장하는데, 이는 도메인 모델의 "ID로만 참조" 원칙을 스키마에도 그대로 적용한 것이다.
- **reservation**: 예약서 한 장이 행 하나다. `(user_id, idempotency_key)` 유니크가 "같은 사용자가 같은 요청 키로 두 건을 만들 수 없다"를 DB 차원에서 보장한다. Redis 멱등성 키가 뚫려도 여기서 막힌다. `(status, created_at)` 인덱스는 만료 스케줄러가 "10분 지난 PENDING"을 훑을 때 전체 테이블을 뒤지지 않게 해준다.

날짜를 기간(check_in~check_out)으로 저장하는 reservation과 날짜 단위 행으로 저장하는 room_daily_inventory가 공존하는 구조인데, 예약은 "한 건의 계약"이라 기간이 자연스럽고, 재고는 "날짜별 수량 차감"이 필요해서 일 단위가 자연스럽다. 예약 생성 시 기간을 날짜 목록으로 풀어(occupiedDates) 재고 행들을 차감하는 것으로 둘이 연결된다.
