...
**CHECK 제약과 시드는 SQL을 그대로 쓴다.** `op.execute(...)`로 재귀 CTE를 넣으면 Java 판에서 쓰려던 SQL이 문자 그대로 옮겨진다. 1.7절의 제약 7종도 마찬가지다 — **DB에 거는 것이라 언어가 바뀌어도 같다.**
...
│
├── common/                      ← 읽기만 한다 (F01 소유 아님). 아무에게도 의존하지 않는다
│   ├── config.py                pydantic-settings. pms.* 키가 여기 산다
│   ├── clock.py                 Clock 프로토콜 + Asia/Seoul 고정 구현 (D2)
│   ├── errors.py                에러 코드와 예외 계층
│   ├── error_handlers.py        예외 → HTTP 변환. 도메인은 상태 코드를 모른다
...
│   ├── adapter/persistence/
│   │   └── inventory_repository.py   ★ 재고 차감·복원 조건부 UPDATE
...
└── reservation/                 ← F01 소유
    ...
    │       └── check_in_out.py
    ├── adapter/
    │   ├── web/                 라우터, 요청·응답 스키마 (도메인 모델과 다른 클래스)
    │   ├── persistence/         리포지토리 구현. ★ 상태 전이 조건부 UPDATE
    │   ├── lock/                RedisLockAdapter(acquire_all), NoOpLockAdapter
    │   ├── idempotency/         Redis SET NX
    │   ├── payment/             모의 결제 (2.3절)
    │   ├── scheduler/           만료 스케줄러 (유스케이스를 부르기만 한다)
    │   └── actuator/            GET /api/internal/config (D26)
    └── container.py
...
def create(self, command: CreateReservationCommand) -> ReservationResult:
    ...
    for hook in self._pre_check_hooks:                # [PreCheckHook] — 락보다 앞 (D23)
        hook.check(command)
    with self._lock.acquire_all(keys, wait, ttl):     # 락 (안에서 정렬한다, D10)
...
**설정은 한 곳에서 읽어 프로바이더로 나눠준다.** 특히 `/api/internal/config`가 노출하는 값은 **락 어댑터가 실제로 쓰는 그 프로바이더**에서 와야 한다. 아니면 "꺼졌다고 보고하는데 실제로는 도는" 상태가 만들어진다 (D26, T89).
