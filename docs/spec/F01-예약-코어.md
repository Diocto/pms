...
| `V051__시드_호텔_객실타입.sql` | 호텔 2곳, 객실타입 5종 |
| `V052__시드_일별_재고.sql` | 적용일부터 90일치 재고 행을 재귀 CTE로 생성 |

시드 재고는 `CURDATE()`를 기준으로 90일을 만든다. 파일 내용이 고정이라 Flyway 체크섬은 안전하고, 언제 적용해도 "오늘부터 90일"이 된다. 대신 **통합·동시성 테스트는 이 시드에 의존하지 않는다.** 각 테스트가 자기 데이터를 직접 넣고 지운다 (`tdd.md`: 테스트 간 데이터 격리는 각 테스트가 책임진다). 시드는 로컬 실행과 k6 부하테스트를 위한 것이다.

시드 규모와 구성은 D13에서 확인받는다.
...
| 재고 행이 없는 날짜 (90일 시드 밖) | 409 `INSUFFICIENT_INVENTORY` | 트랜잭션 롤백. 갱신 행 수가 모자란 것으로 같이 잡힌다 |
...
**"재고 행이 없는 날짜"에 대한 주의.** 90일 시드 밖 날짜는 "아직 판매를 열지 않은 것"과 "매진"이 구분되지 않고 둘 다 재고 부족으로 응답한다. 판매 여부를 별도 컬럼으로 관리하는 것은 범위에서 뺐다. 잘못 파는 것보다 안 파는 쪽이 안전하므로 이 방향의 실패는 허용한다.
...
### D13. 시드 데이터를 호텔 2곳·객실타입 5종·90일치로 하고, 경합 실험용 소량 재고 타입을 하나 둔다

- **결정:** 호텔 2곳(서울 1, 부산 1), 객실타입 5종(서울 3종, 부산 2종), 적용일부터 90일치 재고를 넣는다. 5종 중 하나는 `total_quantity`를 **10**으로 작게 두고 나머지는 50~200으로 둔다.
- **이유:** 호텔이 둘이어야 "멀티 호텔 플랫폼"이라는 00 문서 서사가 성립하고, 타입이 5종이면 F03 검색이 필터링을 보여줄 만큼은 된다. 90일은 예약 가능 기간으로 자연스럽고 재고 행이 450개라 부담이 없다. **소량 재고 타입을 하나 두는 것이 핵심이다.** 부하테스트에서 100명이 잔여 10을 두고 경합하는 장면을 만들려면 그런 객실타입이 시드에 있어야 한다. 재고가 전부 넉넉하면 경합이 일어나지 않아 아무것도 증명하지 못한다.
- **트레이드오프:** 시드가 고정이라 다양한 부하 시나리오를 만들려면 k6 쪽에서 데이터를 더 넣어야 한다. 90일 밖 날짜는 "판매 전"과 "매진"이 구분되지 않고 둘 다 재고 부족으로 응답한다(잘못 파는 것보다 안전한 방향이라 허용한다).
- **대안이었던 것:** 호텔 1곳(서사가 무너져 기각), 365일치(행이 5배인데 얻는 것이 없어 기각), 재고를 전부 넉넉하게(경합 시나리오를 못 만들어 기각).
- **확인 질문:** 이 규모와 구성(특히 경합용 소량 재고 타입 1종)으로 시드를 넣어도 괜찮습니까?
...
| 90일 시드 밖 날짜의 "판매 전"과 "매진" 구분 | 구분하지 않음. 둘 다 재고 부족 응답(fail-closed) | UC-2 실패 시나리오, D13 |
