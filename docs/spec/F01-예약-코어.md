...
- **수정이 필요한 공유 파일:** 두 곳이며, 둘 다 `parallel-work.md`의 공유 파일 절차대로 **이 변경만 담은 작은 커밋을 `main`에 먼저 반영**하고 다른 세션이 rebase로 받아가게 한다.
  ...
  | `src/main/resources/application.yml` | `pms.reservation.hold-minutes`, `pms.reservation.expire-scan-interval`, `pms.lock.enabled`, `pms.lock.wait-millis`, `pms.lock.ttl-seconds`, `pms.payment.decline-rate` 추가 | D10, D11, D17, UC-3 모의 결제 규약 |
...
**컬럼 값은 enum 이름 그대로**의 대문자 문자열이다. `from_status`·`to_status`는 `PENDING`·`CONFIRMED`·`CHECKED_IN`·`CHECKED_OUT`·`CANCELLED`·`EXPIRED`·`NO_SHOW`, `event`는 `CONFIRM`·`PAYMENT_FAILED`·`CANCEL`·`EXPIRE`·`CHECK_IN`·`CHECK_OUT`·`NO_SHOW`다. **전이 순서는 `occurred_at`이 아니라 `id` 순번으로 판단한다.** 같은 트랜잭션 안에서 읽은 `now`를 쓰므로 `occurred_at`이 같은 값일 수 있다.

**이 테이블로 증명할 수 있는 것 세 가지.** 동시성 테스트와 F04 부하 검증이 그대로 쓴다. 셋 다 **결과가 비어 있어야 통과**다.

```sql
-- (1) 재고 복원이 예약당 정확히 한 번인가
--     복원을 일으키는 이벤트는 정확히 이 셋이다 (전이 표 참고)
...
--     결과가 허용 8행의 부분집합이어야 한다
SELECT DISTINCT from_status, event, to_status
  FROM reservation_status_history;
```

(3)에 대한 주의. 이 테이블은 **거부된 시도를 기록하지 않으므로**(D19) "금지된 전이가 시도됐다가 막혔다"는 증명하지 못한다. 그건 전이 표 49칸 전수 테스트의 몫이다. 다만 **"표 밖의 전이가 실제로는 한 번도 일어나지 않았다"는 증명된다.** 시도의 부재가 아니라 결과의 부재이며, 운영 데이터로 하는 증명이라는 점에서 값이 다르다.
...
**예약 생성은 `checkOut > today`를 요구한다** (D21). `checkIn >= today`가 아니다. 체크인일이 지났어도 체크아웃일이 남았으면 **진행 중인 투숙**이고, 체크인·노쇼가 바로 그 구간의 사건이라 허용해야 한다. 위 표의 체크인·노쇼 시연 날짜가 전부 이 조건을 통과한다. 반대로 `2026-08-01`~`08-03`처럼 이미 끝난 숙박은 거부된다.
...
**`price_per_night`에는 실제로 청구한 단가가 들어간다.** 특가 예약이면 특가 단가다. 정가를 넣으면 예약서와 사용권의 금액이 달라지고, D6이 스냅샷을 저장한 이유(사후 백필 불가)가 무너진다. 정가는 `room_type.base_price`에 그대로 있고 이 프로젝트에서는 변하지 않으므로(00 D1, 관리자 CRUD 제외) 할인폭이 필요하면 언제든 계산할 수 있다. 그래서 정가용 컬럼을 따로 두지 않는다.
...
**모의 결제의 동작 규약.** 실제 PG를 붙이지 않으므로(00 문서 제외 항목) 내부 모의 모듈이 대신한다. 그 동작을 여기 정의한다.
...
기본값을 `0.0`으로 두는 이유는 **비결정성은 명시적으로 켜야 하기 때문이다.** 기본이 확률적이면 테스트가 가끔 깨지고, 그때 원인이 경합인지 결제인지 구분되지 않는다. 결제 실패 분기는 유스케이스 테스트가 가짜 결제 포트로 항상 검증하므로 기본값이 `0.0`이어도 죽은 코드가 되지 않는다.

`1.0`을 결정적으로 만들어 두는 이유는 **결제 실패 분기만 따로 검증하는 회차**를 돌 수 있게 하기 위해서다. 고정값이었다면 그 분기를 통합·부하 수준에서 검증할 방법이 없다.
...
`discounts`가 비어 있으면 정가 예약이다. **할인은 명시적으로 요청해야만 적용된다.** 이것이 중요한 이유는, 요청 없이 `(객실타입, 날짜)`가 일치한다는 이유로 자동 적용하면 **일반가로 예약하려는 사용자까지 특가를 소진시키고, 특가가 열린 날짜에는 일반 예약이 아예 불가능해지기 때문이다.**
...
**해석과 훅은 역할이 다르다.** `DiscountResolver`는 **질의**(값을 읽어 단가를 돌려줌)이고 `ReservationCreationHook`은 **명령**(재고를 깎고 사용권을 만듦)이다. 순서는 `해석 → 가격 확정 → 예약 INSERT → 훅`이다. 가격이 INSERT 전에 확정돼야 하므로 해석이 훅보다 앞설 수밖에 없다.

둘이 어긋날 걱정은 하지 않아도 된다. 같은 트랜잭션 안에서 같은 행을 읽고 쓰기 때문이다. 해석 시점과 훅 시점 사이에 특가가 매진되면 **훅의 조건부 UPDATE가 0건을 반환하고 예외를 던져 트랜잭션 전체가 롤백된다.** 가격만 할인가로 잡히고 재고는 안 깎이는 상태가 생길 수 없다.

**확장 지점의 호출 순서.** 넷이 예약 생성의 서로 다른 시점에 걸린다.
...
`PreCheckHook`이 **락보다 앞**이라는 점이 중요하다. 매진 요청을 거르는 것이 목적인데 락을 세 개 잡고 나서 거르면 아끼려던 비용을 이미 치른 뒤다. 거부되면 풀 락도 없어 정리 경로가 가장 짧다. 멱등성 키는 **남긴다** — 재고 부족과 같은 성격이라 같은 키로 다시 와도 결과가 같기 때문이다.

`DiscountResolver`는 락 뒤에 둔다. 가격을 확정하려면 어떤 특가를 쓸지가 정해져 있어야 하고, 그 판단이 재고 확보와 같은 구간 안에 있어야 하기 때문이다.
...
- **확인 질문:** ~~쿠폰 필드 없이 주문서 구조와 가격 계산 분리만으로 00 D5의 확장 지점 조건을 충족한 것으로 봐도 괜찮습니까?~~ → 아래 개정을 보고 판단해 주십시오.

**개정 (2026-08-15, 보고서 제출 후).** **`discounts` 자리를 지금 만든다.** 위 결정을 뒤집는다.

원래 "쿠폰 필드는 지금 만들지 않는다"고 한 근거는 **"항상 비어 있는 필드는 문서가 아니라 잡음"**이었다. 그 근거가 사라졌다. F02가 특가 사용권을 붙이면서 **실제 사용자가 생겼기 때문이다.** 이제 그 필드는 죽은 필드가 아니다.

자리가 없으면 F02는 `(객실타입, 날짜)`가 열린 특가와 일치하는지로 **추론**해야 하는데, 그건 두 가지를 망가뜨린다. **일반가로 예약하려는 사용자가 자기도 모르게 특가를 소진시키고, 특가가 열린 날짜에는 일반 예약이 아예 불가능해진다.** 사용자가 고르지 않은 할인이 적용되는 것은 그 자체로 결함이다.

D12가 예상한 상황이 정확히 왔다 — "쿠폰이 들어오면 파라미터를 추가한다"고 썼고, 지금이 그 시점이다. 다만 **판단 자체는 그대로 유효했다.** 사용자가 없던 시점에 필드를 만들지 않은 것은 옳았고, 사용자가 생긴 지금 만드는 것도 옳다. 구체적인 모양은 7.5절에 있다.
...
**보고서 제출 후 추가된 항목입니다.** F03 세션과 규칙이 갈리는 지점이 발견되어 PM 제안을 받아 확정했습니다.

- **결정:** 예약 생성 시 `checkOut > today`를 검증한다. 위반은 400. **`checkIn`이 과거인 것은 거부하지 않는다.** 애플리케이션 계층에서만 검사하고 CHECK 제약으로는 걸지 않는다.
- **이유:** 원안은 **아무 검증도 하지 않는 것**이었다. 그러면 `2026-08-01` 체크인, `08-03` 체크아웃처럼 **이미 끝난 숙박을 새로 예약할 수 있다.** 죽은 데이터이고, 리뷰에서 "왜 어제 방을 예약할 수 있나"가 반드시 나온다. 그렇다고 흔한 규칙인 `checkIn >= today`를 걸면 **체크인과 노쇼를 시연할 수 없다.** 두 사건 모두 체크인일이 지난 예약에서 일어나기 때문이다(시드 계약의 노쇼 시연 날짜가 `checkIn = 08-14`인 이유다).

  `checkOut > today`가 둘을 동시에 만족한다. 그리고 이건 우회로가 아니라 **도메인적으로 더 정확한 규칙이다.** 체크인일이 지났지만 체크아웃일이 남은 예약은 *진행 중인 투숙*이고 실제로 존재하는 상태다. 체크아웃일까지 지난 예약은 아무 사건도 일어날 수 없다. 경계를 "숙박이 끝났는가"에 두는 것이 "숙박이 시작됐는가"에 두는 것보다 옳다.
- **트레이드오프:** **이 규칙만 DB 최후 방어선이 없다.** 다른 규칙은 전부 CHECK가 받쳐주는데(5.3절), 여기는 애플리케이션 검증 한 겹뿐이다. MySQL의 CHECK 제약에는 `CURDATE()` 같은 비결정적 함수를 쓸 수 없어서 원리적으로 불가능하다. 트리거로는 가능하지만 디버깅이 어렵고 마이그레이션에서 관리하기 나빠 더 나쁜 선택이다. 그리고 시드 범위 앞부분(`2026-08-01`~`08-15`)의 재고가 사실상 팔 수 없는 재고가 된다. 마지막으로, 이 규칙은 **시간이 흐르면 기존 예약에서 성립하지 않게 되므로** 불변식으로 쓸 수 없고 생성 시점 선행 조건으로만 존재한다.
- **대안이었던 것:** 아무 검증도 안 하기(원안. 끝난 숙박 예약을 허용해 기각), `checkIn >= today`(체크인·노쇼 시연이 불가능해져 기각. UC-6 검증이 통째로 막힌다), CHECK 제약으로도 걸기(MySQL이 `CURDATE()`를 CHECK에 허용하지 않아 불가능), 검증을 F03 검색에만 두고 예약은 열어두기(검색을 안 거치고 예약 API를 직접 부르면 뚫려 기각).
- **F03과의 관계:** F03은 검색에서 `checkIn >= today`로 **더 좁게** 막는다. 두 규칙은 모순이 아니라 층이 다르다. **검색은 "고를 수 있는 것"을 보여주고 예약은 "만들 수 있는 것"을 받는다.** 사용자 진입점은 더 좁아도 되고, 그래야 시드 시작일 이전 조회가 "매진"이라는 틀린 안내로 나가지 않는다.
- **확인 질문:** 예약 생성 조건을 `checkOut > today`로 두고, 이 규칙만 DB 최후 방어선 없이 애플리케이션 검증 한 겹으로 가도 괜찮습니까?

### D22. 할인 단가는 할인의 소유자가 정하고, F01은 포트로 받아 그대로 스냅샷한다

**보고서 제출 후 추가된 항목입니다.** F02 세션의 질문(특가 단가가 `price_per_night`에 반영되는가)을 받아 확정했습니다.

- **결정:** `DiscountResolver` 포트를 F01이 정의하고 F02가 구현한다. F01의 `ReservationPricing`이 그 결과를 받아 단가를 정하고, **`reservation.price_per_night`에는 실제로 청구한 단가(특가면 특가)를 저장한다.** 해석에 실패하면 정가로 넘어가지 않고 400으로 거절한다.
- **이유:** 대안 (다)가 "F01은 정가만 기록하고 금액 불일치를 문서화한다"였는데, **이건 D6의 존재 이유를 무너뜨린다.** D6에서 단가 스냅샷을 저장한 근거는 "당시 가격은 어디에도 남아 있지 않고 사후 백필이 불가능하다"였다. 그런데 정가를 박아두면 **그 스냅샷이 실제로 청구한 금액이 아니다.** 틀린 값을 되돌릴 수 없게 저장하는 것이고, 같은 거래의 금액이 예약서와 사용권 두 곳에서 달라진다. 백필 불가라는 성질 때문에 이건 나중에 고칠 수도 없다.

  F01이 직접 할인가를 계산할 수는 없다. 할인의 내용을 모르기 때문이고, 알면 소유 경계가 무너진다. 그래서 **값은 소유자가 주고 계산 권한은 F01이 갖는다.** 이러면 D12의 "가격 계산은 도메인 서비스 한 곳"이 유지되면서 할인이 붙는다. 00 D5의 조건("도메인 계층 추가만으로 구현")과 정확히 맞는 형태이기도 하다.

  해석 실패를 400으로 막는 이유는 **조용히 정가로 넘어가면 사용자가 기대한 금액보다 더 청구하기 때문이다.** 돈이 걸린 판단은 fail-closed여야 한다.
- **트레이드오프:** 확장 지점이 하나에서 **둘(질의용 `DiscountResolver` + 명령용 `ReservationCreationHook`)로 늘었다.** F01에 record 3개·enum 1개·인터페이스 1개가 추가되고, 가격 계산 경로에 해석 단계가 끼어든다. 마감이 8/17인 일정에서 순수한 추가 작업이다. 그리고 **F02가 없으면 이 포트도 죽은 확장 지점**이라, D20에서 이미 한 번 인정한 D12 원칙(빈 추상화 금지)의 예외를 한 번 더 두는 셈이다. 또 F01이 `PROMOTION`이라는 낱말을 `DiscountType`에 갖게 되는데, 이건 F02 코드에 대한 의존은 아니지만 어휘 수준의 결합이다.
- **대안이었던 것:** (다) 정가만 기록하고 불일치를 문서화(위 이유로 기각. 문서화로 덮을 수 있는 종류의 문제가 아니다), (가) 커맨드에 확정 단가를 직접 실어 F01이 그대로 저장(가장 싸지만 F01의 가격 계산이 우회 가능해지고 "가격 계산은 한 곳"이라는 D12 주장이 깨져 기각. 다만 시간이 부족하면 이쪽으로 후퇴할 수 있다), 정가용 컬럼을 따로 추가해 둘 다 저장(`room_type.base_price`가 이 프로젝트에서 변하지 않아 언제든 읽을 수 있으므로 불필요하다고 판단해 기각).
- **확인 질문:** 확장 지점이 둘로 늘어나는 것을 감수하고, 예약서에 **실제 청구 단가**를 기록해도 괜찮습니까? (시간이 부족하면 대안 (가)로 후퇴할 수 있습니다)
...
- **이유:** F02의 특가 게이트(매진 요청을 Redis 카운터로 미리 자르는 1차 방어)가 놓일 자리가 없어졌다. 기존 확장 지점 둘은 **모두 트랜잭션 안**이라, F02 코드가 실행될 수 있는 가장 이른 시점이 이미 락을 잡고 트랜잭션을 연 뒤다. 거기서 게이트를 돌리면 아끼려던 비용을 이미 다 치른 뒤이고, **트랜잭션 안에서 원격 호출을 하지 않는다**는 이 프로젝트의 규칙에도 걸린다.

  정확성은 이 훅이 없어도 지켜진다. 조건부 UPDATE와 CHECK가 책임진다. **잃는 것은 처리량이다.** 잔여 20에 8,000건이 오면 7,980건이 락을 잡고 트랜잭션을 열고 0건을 받고 롤백한다.

  비용이 작다는 것이 결정의 근거다. **나는 이미 트랜잭션 밖 단계(멱등 키 선점 → 검증 → 락 획득)를 갖고 있어서 끼울 자리가 그대로 있다.** 인터페이스 하나와 호출 지점 하나, 그리고 거부 시 정리 경로는 기존 "재고 부족" 경로를 그대로 쓴다. 아직 구현을 시작하지 않았으므로 지금이 가장 싼 시점이기도 하다.

  **F02가 제안한 위치(락 획득 *후*)를 바꿨다.** 매진 요청을 거르는 것이 목적인데 락을 셋 잡고 나서 거르면 아끼려던 비용을 이미 치른 뒤다. 락 앞에 두면 거부 경로에 풀 락이 없어 정리도 가장 짧다.
- **트레이드오프:** **F01의 확장 지점이 넷이 됐고, 넷 모두 구현자가 F02 하나뿐이다.** 이건 명백한 설계 냄새다. D12에서 "구현체 없는 추상화는 과잉 설계"라며 기각했던 것을 D20·D22에 이어 세 번째로 예외 처리하는 셈이다. 예약 생성의 확장 지점이 늘수록 F01 유스케이스가 "여러 훅을 순서대로 부르는 틀"에 가까워지고, 순서 자체가 계약이 되어 문서 없이는 읽을 수 없게 된다. **다섯 번째가 필요해지면 그때는 훅을 더 만들 것이 아니라 F02가 자기 진입점을 갖는 쪽을 다시 검토해야 한다.** 그 선을 여기 적어둔다.
- **대안이었던 것:** A안 — 그냥 두기(정확성에 영향이 없고 계약이 안 늘지만, 특가만 2층 방어가 되어 "3층 방어" 서사에 예외가 하나 더 붙고 F04가 "매진 트래픽을 DB 앞에서 자르면 처리량이 얼마나 달라지는가"를 측정할 수 없게 된다), C안 — F02가 서블릿 필터로 끼어들기(본문을 다시 파싱해야 하고 F02가 F01의 웹 계층에 침범하는 형태라 F02가 스스로 기각했다. 동의한다), F02가 자기 엔드포인트를 갖기(초안 형태. 예약 생성 로직이 두 벌이 되거나 락·트랜잭션 순서가 뒤집혀 기각).
- **확인 질문:** 확장 지점이 넷으로 느는 것을 감수하고 사전 검사 훅을 추가해도 괜찮습니까? (정확성 문제가 아니라 처리량과 서사 일관성의 문제입니다)
