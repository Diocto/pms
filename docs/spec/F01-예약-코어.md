...
### D34. 컨테이너 싱글턴은 전부 잠금 있는 판(ThreadSafeSingleton)으로 만든다 — ✅ **구현 중 결정 (기록, 2026-08-16)**

**F04 부하테스트가 실측으로 잡아 넘긴 문제를 닫는 결정입니다** (되돌리는 비용이 코드 몇 줄이라 정하고 진행하며 기록).
...
- **검증:** 테스트 T90(스레드 16개 동시 첫 접근 → 엔진 정확히 1개; 고치기 전엔 16개가 만들어짐)과 T90b(잠금 없는 Singleton이 하나라도 남으면 빨간불이 되는 구조 검사).
