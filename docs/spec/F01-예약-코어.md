...
- 상태: **재작성 완료, 재승인 대기** — 스택 전환(FastAPI)에 맞춰 다시 씀
- 스택: Python + FastAPI + SQLModel + Dependency Injector + Alembic + MySQL 8.4 + Redis 7.4
- 작성일: 2026-08-15
- 최종 개정: 2026-08-15 (4단 구조 재정렬 → 관리자 판단 반영 → **스택 전환 반영**)
- 승인일:
...
- **읽기만 하는 모듈:** `app/common/**` (에러 계층, 응답 포맷, `Clock`, 설정). **`common`은 아무에게도 의존하지 않는다.**
...
- **수정이 필요한 공유 파일:** 두 곳이며, 둘 다 `parallel-work.md`의 공유 파일 절차대로 **이 변경만 담은 작은 커밋을 `main`에 먼저 반영**하고 다른 세션이 rebase로 받아가게 한다.
  ...
  | 파일 | 변경 | 근거 |
  |---|---|---|
  | `app/common/clock.py` | 시스템 기본 존 대신 `Asia/Seoul` 고정 | D2 |
  | `app/common/config.py` | `pms.` 키 6개 추가 (pydantic-settings) | D10, D11, D17, 2.3절 |

선행 커밋에 들어갈 설정은 정확히 이것이다. 값은 환경변수로 뒤집을 수 있어야 한다(`PMS_LOCK_ENABLED` 등).
  ...
  **설정을 한 곳에서 읽어 프로바이더로 나눠주는 것이 중요하다.** 같은 값을 두 곳에서 따로 읽으면 어긋나고, 특히 `/api/internal/config`가 노출하는 값이 락 어댑터가 쓰는 값과 갈리면 **"꺼졌다고 보고하는데 실제로는 도는"** 상태가 만들어진다 (D26).
...
```python
# LockPort 구현 안. 부르는 쪽은 정렬할 기회조차 없다 (D10)
def acquire_all(self, keys: list[str], wait: timedelta, ttl: timedelta):
    for key in sorted(keys):        # 정렬은 여기서만 일어난다
        ...
```

**정렬을 하는 자리를 한 곳으로 좁힌 것이 핵심이다.** 관리자 지시가 "바깥에서 실수해도 안에서 정렬하도록"이었고, 락 포트가 받은 목록을 스스로 정렬하면 **호출부는 정렬을 빠뜨릴 수 없다.** "어차피 오름차순으로 들어온다"는 가정에 기대지 않는다 — 그 가정은 리팩터링 한 번에 깨진다.

**같은 목록을 재고 차감 UPDATE의 실행 순서에도 쓴다.** 락 순서만 정렬하고 DB 쪽을 안 하면 같은 데드락이 InnoDB 행 락에서 난다. 특히 **락을 껐을 때(K2) 방어선이 DB 행 락뿐이라 그대로 드러난다.**
...
class CreateReservationCommand:
    ...
    idempotency_key: str
    line: OrderLine
    discounts: list[DiscountRef] = field(default_factory=list)   # 지금은 0개 또는 1개
...
**설정 노출 키가 조작자가 셸에 치는 이름과 달라지면 재현이 막힌다.** `/api/internal/config`는 **스프링 설정 키 이름 그대로의 평평한 문자열**을 쓴다(D26). 번역하는 순간 리포트의 설정 표와 실제 값이 갈린다.
...
**진행 상황:** D1·D4·D7·D10·D17·D22·D25·D26·D27 **판단 완료.** D24는 스택 전환으로 폐기되고 D26이 대체했습니다. 남은 것은 **[확인]·[참고] 17개의 일괄 동의**입니다.
...
- **응답 계약:**
  ...
  **키는 설정 키 이름 그대로의 평평한 문자열이다. 중첩하지 않는다.** 이름을 바꾸지 않으면 번역 과정에서 어긋날 자리가 없고, 셸에서 중첩 JSON 파싱은 깨지기 쉽다. **필수는 `pms.lock.enabled` 하나**이고 나머지는 있으면 좋은 것이다. **F01이 소유한 키만 싣는다** — `pms.promotion.*`은 F02가 자기 라우트나 프로바이더를 추가한다.

- **가장 중요한 제약 — 검증 장치가 검증 대상과 같은 곳을 봐야 한다.**

  **이 값은 락 코드가 실제로 쓰는 그 설정 프로바이더에서 읽는다.** 따로 읽으면 **"config는 false라고 하는데 락은 true로 도는"** 상태가 만들어지고, **그러면 이 장치가 막으려던 거짓 통과를 이 장치 자신이 만든다.** Dependency Injector에서 **같은 `Configuration` 프로바이더를 락 어댑터와 이 라우트에 함께 주입**하면 구조적으로 어긋날 수 없다. 테스트로도 못을 박는다(T87~T89).
...
- **위치:** `app/reservation/adapter/actuator/`. `pms.` 키 6개가 전부 F01 소유(`lock`·`reservation`·`payment`)라 공유 파일이 아니다. **락 어댑터를 만드는 3회차에 함께 만든다** — 같은 회차에 만들어야 "같은 프로바이더를 공유한다"는 제약이 구조적으로 지켜진다.
