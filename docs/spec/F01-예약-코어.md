...
- 상태: **승인 진행 중** — D1·D4·D17·D22 확정, **D7·D10 확인 대기**
- 작성일: 2026-08-15
...
- **수정이 필요한 공유 파일:** 두 곳이며, 둘 다 `parallel-work.md`의 공유 파일 절차대로 **이 변경만 담은 작은 커밋을 `main`에 먼저 반영**하고 다른 세션이 rebase로 받아가게 한다.
  ...
  | `src/main/resources/application.yml` | `pms.` 키 6개 추가 + **actuator `env` 엔드포인트 개방** | D10, D11, D17, D24, 2.3절 |

  선행 커밋에 들어갈 `application.yml` 변경은 정확히 이것이다.
  ...
  management:
    endpoints:
      web:
        exposure:
          include: health,info,metrics,prometheus,env   # ← env 추가 (D24)
    endpoint:
      env:
        show-values: ALWAYS        # 이게 없으면 값이 ******로 마스킹된다 (D24)
...
| `confirmation_code` | CHAR(12) | NOT NULL, UK | `K7M2XQ4RN9PH` | **모든 API 경로의 식별자.** 혼동 문자 제외 32자 알파벳으로 만든 12자리 무작위 코드 |
...
```json
{ "confirmationCode": "K7M2XQ4RN9PH", "status": "CONFIRMED", "confirmedAt": "2026-09-01T14:27:11" }
```
```json
{ "confirmationCode": "K7M2XQ4RN9PH", "status": "CANCELLED", "failureReason": "PAYMENT_DECLINED" }
```
...
| 외부 식별자 | **무작위 확인번호 12자** | 열거·총량 추정 차단. 식별자 변경은 API 파괴라 나중에 못 고친다 | `id` 노출(열거 가능), UUIDv4(36자라 전화로 못 부름) | D7 |
...
**진행 상황:** D1·D4·D17·D22 **관리자 판단 완료.** D7·D10 **확인 대기.** 나머지는 동의 여부만 확인하면 됩니다.
...
### D7. 외부에 노출하는 식별자를 무작위 확인번호로 한다 (반영 후보 7: 채택) — ⏳ **확인 대기**

- **결정:** `reservation.confirmation_code`(12자리 무작위 대문자 영숫자, UK)를 두고 **API 경로와 응답에는 이것만 쓴다.** auto-increment `id`는 내부 전용으로 남긴다.
- **무엇인가 (관리자 요청으로 풀어 씀):** 지금 구조면 주소가 `/reservations/1`, `/2`, `/3`이 된다. 두 가지가 샌다. **총량과 속도** — 예약 번호 8,412를 받았다가 하루 뒤 9,105를 받으면 하루 예약량이 약 700건임을 알 수 있다. **열거** — 번호를 1부터 올리면 남의 예약이 순서대로 나온다. 확인번호는 그 자리에 예측 불가능한 문자열을 놓는 것이다. 호텔에서 문자로 오는 그 코드다.

  | 항목 | 값 |
  |---|---|
  | 형식 | 12자리 대문자 영숫자 (`K7M4XQ2NPR9T`) |
  | 알파벳 | 혼동 문자(`0`/`O`, `1`/`I`) 제외 32자 — **전화로 불러줄 수 있어야 한다** |
  | 경우의 수 | 32<sup>12</sup> ≈ 115경. 찍어서 못 맞힌다 |
  | DB | `CHAR(12)` + UK 1개 |
  | `id` | 없애지 않는다. **내부 전용**으로 FK·조인에 그대로 쓴다 |
  | 충돌 | UK 위반을 잡아 최대 3회 재생성 |

  곁들여 — 남의 확인번호로 조회하면 **403이 아니라 404**를 준다. 403은 "그 번호는 있는데 네 것이 아니다"라고 알려주는 셈이라 존재 여부가 샌다.
- **이유:** **이건 나중에 바꿀 수 없다.** 식별자 형식 변경은 API를 파괴하는 변경이라, 처음에 정하지 않으면 영원히 못 고친다. 비용은 컬럼 하나와 UK 하나다.
- **트레이드오프:** 모든 조회가 PK 대신 UK 인덱스를 한 번 더 타고, 코드 생성·충돌 재시도 로직이 추가된다.
- **대안이었던 것:** `id` 그대로 노출(열거 가능, 되돌릴 수 없어 기각), UUIDv4(36자라 전화로 못 부르고 인덱스가 커서 기각), `id` 노출 + 소유자 검증(총량 추정은 여전히 가능해 기각).
- **하위 영향:** **F04가 이 경로로 k6 스크립트 10종을 이미 다 짰다.** 뒤집으면 모든 경로와 응답 파싱이 바뀐다.
- **확인 질문:** 무작위 확인번호를 외부 식별자로 써도 괜찮습니까?
...
### D10. 분산락을 `(객실타입, 날짜)` 단위로 여러 개 잡는다 — ⏳ **확인 대기**
...
### D24. actuator `env` 엔드포인트를 열고, 값 마스킹을 해제한다

**보고서 제출 후 추가된 항목입니다.** F04 세션이 PM을 통해 올린 요청을 받아 판단했습니다.

- **결정:** `application.yml`의 actuator 노출 목록에 `env`를 추가하고, **`management.endpoint.env.show-values: ALWAYS`를 함께 넣는다.** 프로파일로 나누지 않는다. 선행 커밋에 포함한다.
- **이유:** F04의 락 On/Off 대조 시나리오는 **실행 전에 스위치가 실제로 꺼졌는지 확인**해야 한다. F04가 자기 「거짓 통과」 목록에서 가장 위험한 항목으로 꼽은 것이 이거다 — **스위치가 실제로는 안 꺼져서 두 회차가 같은 조건이 되면 결과가 "차이 없음"으로 나오고, 그 결론은 "락이 없어도 되더라"로 읽혀 정반대 주장을 만든다.** F04의 `reset.sh`가 `/actuator/env/pms.lock.enabled`를 읽어 기대값과 다르면 중단하고, **읽지 못해도 중단한다.** `env`가 닫혀 있으면 시나리오가 시작조차 못 한다.

  **`show-values`를 함께 넣는 것이 요점이다.** `include`에 `env`만 추가하면 값이 `******`로 마스킹돼 나올 수 있고, 그러면 F04 스크립트는 "읽지 못했다"로 판정해 중단한다. **엔드포인트는 열렸는데 검증은 여전히 불가능한 상태**가 되는 것이다. 이 설정이 이미 기본값이더라도 명시해서 손해가 없으므로 넣는다.
- **트레이드오프:** **`env`는 설정 전체를 보여주고, 마스킹까지 해제하므로 DB 접속 정보가 그대로 노출된다.** 이 과제는 로컬 전용이고 인증도 `X-User-Id` 헤더로 대체한 상태라 실질 위험은 없지만, **이 설정 그대로 실서비스에 올리면 사고다.** 감추지 않고 여기 적는다 — D21에서 "이 규칙만 DB 최후 방어선이 없다"를 적은 것과 같은 자리다.

  실서비스라면 `env`를 닫거나, 최소한 `show-values`를 `WHEN_AUTHORIZED`로 두고 actuator에 인증을 걸어야 한다.
- **대안이었던 것:**
  - **(나) `local` 프로파일에서만 열기** — 노출은 줄지만 **F04가 어떤 프로파일로 앱을 띄우는지에 의존**해서 또 어긋날 여지가 생긴다. 지금 F04는 `SPRING_APPLICATION_JSON`으로 스위치를 넣고 기본 프로파일로 띄운다. 프로파일 조건을 걸면 "열었다고 생각했는데 안 열린" 상태가 되고, **그건 이 결정이 막으려던 실패 모드와 정확히 같은 종류다.** 기각.
  - **(다) `pms.lock.enabled`만 돌려주는 전용 엔드포인트** — 노출이 최소지만 컨트롤러가 하나 늘고 F04 스크립트도 바꿔야 한다. 노출 축소가 로컬 전용 과제에서 그만한 값을 하지 않아 기각.
  - **(라) `/actuator/info`에 락 상태를 싣기** — `info`는 이미 열려 있고 마스킹도 없어 노출이 늘지 않는다. `InfoContributor` 빈 하나면 된다. **기술적으로 가장 깔끔한 선택이지만** F04가 이미 `env` 경로로 스크립트를 썼고 마감이 이틀이라 채택하지 않았다. **F04가 URL 변경을 감수하겠다면 이쪽이 더 낫다** — 그 판단은 F04에 넘긴다.
- **확인 질문:** 로컬 전용임을 문서에 명시하는 조건으로, actuator `env`를 열고 값 마스킹을 해제해도 괜찮습니까?
...
**현재 대기 중:** D7(무작위 확인번호), D10(날짜 단위 다중 락).
