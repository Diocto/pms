...
| 구성 요소 | 한국어 이름 | 무엇인가 | 왜 이렇게 두나 |
|---|---|---|---|
| Reservation | 예약서 | 누가·어떤 타입을·언제·몇 실·얼마에 + 현재 상태 | 상태 변경 통로는 `handle(이벤트)` 하나뿐. 세터 없음 |
| StayPeriod | 투숙 기간 (VO) | 체크인·체크아웃 날짜 묶음 | "체크아웃 > 체크인" 규칙을 생성 시점에 강제. 날짜 목록 계산(occupiedDates)을 한곳에 |
| ReservationStateMachine | 상태 전이표 | (현재 상태, 이벤트) → 다음 상태 맵 | 예약서는 판단하지 않고 표에 묻는다. if-else 금지 |
| RoomDailyInventory | 일별 재고 | 객실타입 하나의 하루치 잔여 수량 행 | 동시성 제어의 무대. "8/20 디럭스 잔여 3" = 행 하나 |
| Hotel / RoomType | 참조 데이터 | 시드로 넣는 읽기 전용 정보 | 수정 유스케이스 없음 |

| 관계 표기 | 뜻 |
|---|---|
| `*--` (다이아몬드 실선) | StayPeriod는 예약서에 소속된 부품 |
| `..>` (점선 화살표) | ID로만 참조. 객체를 직접 들지 않아 수정·잠금 범위가 애그리거트 안에 갇힌다 |
...
```mermaid
erDiagram
    hotel ||--o{ room_type : "호텔 1 : 객실타입 N (시드)"
    room_type ||--o{ room_daily_inventory : "타입 1 : 날짜별 재고 행 N"
    room_type ||--o{ reservation : "ID만 저장, FK 없음"
    ...
    room_type {
        ...
        varchar name "타입명(디럭스 등)"
        int capacity "정원"
        decimal price_per_night "1박 단가"
        int total_quantity "타입 총 객실 수"
    }
    room_daily_inventory {
        bigint id PK
        bigint room_type_id "객실타입 UK(+stay_date)"
        date stay_date "재고 날짜 UK"
        int total_quantity "그날 판매 가능 총수"
        int remaining "잔여 수량 CHECK>=0 최후방어선"
    }
    reservation {
        bigint id PK
        varchar user_id "예약자 UK(+idem_key)"
        bigint room_type_id "객실타입 ID참조"
        date check_in "체크인"
        date check_out "체크아웃"
        int room_count "예약 실 수"
        decimal total_price "총액 = 일수x단가x실수"
        varchar status "상태(전이 표 참조)"
        varchar idempotency_key "멱등 키 UK 최후방어선"
        datetime created_at "생성 시각 IX(+status) 만료조회용"
    }
```

| 테이블 | 역할 | 핵심 장치 |
|---|---|---|
| hotel, room_type | 시드 참조 데이터. 이 feature에서 쓰기 없음 | |
| room_daily_inventory | 날짜별 잔여 수량. 동시성 제어 대상 | UK(타입,날짜)로 조건부 UPDATE 대상 행이 정확히 하나. CHECK로 음수 재고 저장 불가 |
| reservation | 예약서 한 장 = 행 하나 | UK(user_id, idem_key)로 Redis가 죽어도 중복 예약 불가. IX(status, created_at)는 만료 스케줄러 조회용 |

reservation은 기간(check_in~check_out)으로, room_daily_inventory는 날짜 단위 행으로 저장한다. 예약은 계약 한 건이라 기간이 자연스럽고 재고는 날짜별 차감이 필요해서다. 생성 시 기간을 날짜 목록으로 풀어 재고 행들을 차감하는 것으로 둘이 연결된다.
