...
| `053_seed_hotels_extension` | **시드 확장 (2026-08-16, 관리자 지시)** — 호텔 3~100, 객실타입 294종, 재고 26,460행. 규칙은 1.9절 (2b)(3b) |
| `054_seed_hotel_addresses` | **주소 정정 (2026-08-17)** — 호텔 3~100의 이름·주소를 지역 대역별로 다시 찍는다. 데이터만 바꾸고 스키마·API는 그대로. 규칙은 1.9절 (2b) |
...
**이 절은 F03·F04·F05가 이 문서만 보고 맞출 수 있어야 한다.** 세션마다 다른 날짜나 id를 가정하면 통합 시점에 **조용히 실패한다.** 재고 행이 없는 날짜를 예약하면 응답이 `409 INSUFFICIENT_INVENTORY`인데, 이건 "매진"과 구분되지 않아 정상 동작처럼 보인다. 부하테스트가 성공 0건을 받고 나서야 뭔가 잘못됐음을 알게 된다. 그래서 값을 여기 숫자로 고정한다.
...
### (2b) 확장 호텔 3~100 — 리비전 `053` (2026-08-16, 관리자 지시)

F05의 검색 화면이 "호텔 N곳 조회" 구조라 호텔을 100곳으로 늘렸다. **호텔 1·2와 객실타입 1~5의 위 계약은 한 글자도 바뀌지 않는다** — F04의 부하테스트 시나리오가 그 값을 그대로 쓴다.

호텔 h (3 ≤ h ≤ 100)는 전부 규칙에서 유도된다. F05가 이 규칙만으로 매핑 상수를 생성할 수 있고, 실물 목록은 `GET /api/hotels`(2.1절)가 준다.

**2026-08-17 개정 (리비전 `054`).** 이름·주소가 **h가 속한 지역 대역**에서 유도된다. 규칙 생성이라는 성질은 그대로고 접두 문자열만 대역별로 갈린다. 고친 이유는 검색 화면이 주소를 그대로 노출하는데 98곳이 전부 `서울특별시 테스트구`라 미완성으로 읽혔기 때문이다.

| 항목 | 규칙 | 예 (h=50) |
|---|---|---|
| id | h | 50 |
| name | `<지역> 호텔 ` + 3자리 0채움 h | 제주 호텔 050 |
| address | `<지역 주소 접두> ` + h | 제주특별자치도 제주시 중앙로 50 |

| id 대역 | 지역 | 주소 접두 | 곳수 |
|---|---|---|---|
| 3~23 | 서울 | 서울특별시 중구 세종대로 | 21 |
| 24~36 | 부산 | 부산광역시 해운대구 해운대해변로 | 13 |
| 37~52 | 제주 | 제주특별자치도 제주시 중앙로 | 16 |
| 53~64 | 강원 | 강원특별자치도 속초시 중앙로 | 12 |
| 65~72 | 경기 | 경기도 고양시 일산동구 중앙로 | 8 |
| 73~78 | 인천 | 인천광역시 연수구 컨벤시아대로 | 6 |
| 79~84 | 경주 | 경상북도 경주시 첨성로 | 6 |
| 85~89 | 전주 | 전라북도 전주시 완산구 어진길 | 5 |
| 90~95 | 여수 | 전라남도 여수시 돌산로 | 6 |
| 96~100 | 대구 | 대구광역시 중구 동성로 | 5 |

호텔 1·2는 리비전 `051`의 값 그대로다(서울·부산). 합치면 서울 22곳, 부산 14곳이다. 분포 근거는 `docs/reports/F07-지역검색-기획.md` D5.

(원안 기록) 리비전 `053`이 찍었던 값: name `호텔 ` + 3자리 0채움 h, address `서울특별시 테스트구 예약로 ` + h.

### (3b) 확장 객실타입 — 리비전 `053`

객실타입 id는 **h × 1000 + n** (n = 1, 2, 3)이고, 구성은 전 호텔이 같다.

| n | name | capacity | total_quantity | base_price |
|---|---|---|---|---|
| 1 | 스탠다드 | 2 | 50 | 150000 |
| 2 | 디럭스 | 3 | 30 | 250000 |
| 3 | 스위트 | 4 | 10 | 400000 |

예: 호텔 50의 객실타입은 50001(스탠다드)·50002(디럭스)·50003(스위트)다. **n=3 스위트(10실)가 호텔마다 있어** 어느 호텔로도 경합 시나리오를 만들 수 있다.

이 id 대역(3001~100003)은 테스트 전용 대역(900~999, 9300번대, 93200번대)과 겹치지 않는다.

**규모와 실행 시간:** 전체 호텔 100곳, 객실타입 299종(기존 5 + 확장 294), 재고 26,910행(기존 450 + 확장 26,460). 리비전 `053` 하나의 실행 시간은 로컬 실측 **약 0.2초**, `base`부터 `head`까지 전체 마이그레이션이 약 0.5초다 — 재시드 절차에 유의미한 시간이 더해지지 않는다.
...
행 수는 기존 5종 × 90일 = **450행**, 확장(리비전 `053`)을 합치면 299종 × 90일 = **26,910행**이다. 확장 재고도 같은 고정 날짜 범위(2026-08-01 ~ 2026-10-29)이고 초기 `remaining`은 전부 `total_quantity`와 같다.
...
|---|---|---|---|---|
| 호텔을 고른다 | 검색 화면의 호텔 목록 | GET | `/api/hotels` | 호텔 목록 (2026-08-16 추가, 익명) |
| 방을 고르고 "예약하기" | 날짜·인원·객실 수 입력 → 제출 | POST | `/api/reservations` | UC-2 (2.2) |
| 내 예약 전체를 본다 | 예약 내역 화면 | GET | `/api/reservations` | 목록 조회 (2026-08-16 추가, D34) |
...
### 목록·호텔 API 계약 (2026-08-16 추가)

**`GET /api/reservations`** — 내 예약 목록 (D34).

- 헤더 `X-User-Id` 필수. 없으면 400 `INVALID_REQUEST`
- 응답: 그 사용자의 예약 **배열**. 원소는 단건 조회의 `ReservationResponse`와 같은 형태(camelCase)이고, **최신순**(생성 시각 내림차순, 같으면 id 내림차순)
- 선택 쿼리 `?status=` — 전이 상태 enum(`PENDING`·`CONFIRMED`·`CHECKED_IN`·`CHECKED_OUT`·`CANCELLED`·`EXPIRED`)만 받고, 밖의 값은 400 `INVALID_REQUEST`
- 예약이 없으면 404가 아니라 **빈 배열**이다
- 단건 조회의 404 존재 은닉과 충돌하지 않는다 — 소유자 필터가 조회 조건에 박혀 있어 남의 예약은 애초에 결과에 없다

**`GET /api/hotels`** — 호텔 목록과 객실타입 매핑 (F05 검색 화면용).

- 검색과 같은 **익명 경로**다. `X-User-Id`를 받지 않는다
- 응답: `{ "hotels": [ { "hotelId", "name", "address", "roomTypes": [ { "roomTypeId", "name", "capacity", "totalQuantity", "basePrice" } ] } ] }`
- 정렬: 호텔 id 오름차순, 그 안에서 객실타입 id 오름차순
- 정적 마스터 정보만 준다. **잔여 수량은 없다** — 그건 `GET /api/availability`(F03)의 일이다
...
| 다른 사용자의 예약 | 404 `RESOURCE_NOT_FOUND` | **결제를 호출하지 않는다.** 403이 아니라 404 — 확인번호의 존재를 알려주지 않는다 (D33, 관리자 지시) |
...
**세션을 첫 인자로 받는 것이 계약의 핵심이다.** 전환 전 스택(Spring)은 트랜잭션이 스레드에 매여 있어 훅이 아무것도 안 해도 같은 트랜잭션에 들어갔다. SQLAlchemy는 그렇지 않다 — **세션을 넘겨주지 않으면 구현자가 자기 세션을 열 수 있고, 그러면 트랜잭션이 둘로 갈라진다.**
...
**유스케이스를 두 개로 쪼개지 않는다.** 전환 전 스택에서는 락을 트랜잭션 밖에서 잡으려면 프록시 때문에 빈을 둘로 나눠야 했지만, SQLAlchemy는 세션이 명시적이라 그 강제 분할이 없다. SQLAlchemy는 세션이 명시적이라 **한 함수 안에서 순서가 그대로 읽힌다.**
...
**이 한 조각이 3.3절(락 순서)·3.6절(확장 지점 호출 순서)·D23(사전 검사 위치)을 전부 눈으로 확인시켜 준다.** 한 함수에서 순서가 읽힌다는 것 자체가 이 스택을 고른 이유 중 하나다 (ADR-0050).
...
**전환 전 스택의 함정 목록을 그대로 옮기지 않았다.** 사라진 것, 이름만 바뀐 것, 새로 생긴 것을 나눠 적는다. **"옛 스택의 함정이니까 지운다"로 처리하면 진짜 위험한 것이 같이 지워진다.**
...
**확장 지점이 세션을 인자로 받지 않으면 트랜잭션이 갈라진다.** 전환 전 스택은 트랜잭션이 스레드에 매여 있어 훅이 자동으로 같은 트랜잭션에 들어갔다. SQLAlchemy는 아니다. 세션을 안 주면 구현자가 자기 세션을 열 수 있고, **"함께 커밋하거나 함께 롤백한다"가 조용히 깨진다.**
...
### D29. 투숙 기간에 상한 30박을 둔다 — ❌ **뒤집음 (관리자 지시, 2026-08-16)**

**관리자 결정으로 상한을 뺐다:** "n박은 제한 없이 가자. 호텔 관리자 측에서 예약 가능한 일수를 어차피 조정할 거라 제한은 안 둬도 될 듯하다."

- **뒤집은 뒤의 울타리:** 코드 상한 대신 **재고를 열어둔 날짜 범위**가 기간을 정한다. 재고 행이 없는 날짜는 조건부 UPDATE가 0행을 갱신해 "방 없음"(409)으로 끝나므로, 호텔이 열지 않은 기간의 예약은 어차피 성립하지 않는다 — 관리자 전제가 코드에서 그대로 성립함을 확인했다.
- **남는 값:** 아주 긴 기간을 넣은 요청 하나가 날짜 수만큼 락 키를 쥐었다가 재고 없음으로 실패하는 짧은 사이, 같은 객실타입의 정상 요청이 락을 기다릴 수 있다. 데이터는 안 깨지고(2·3층 방어), 실패 즉시 락이 풀리므로 창이 짧다. 이 정도는 감수한다는 결정이다.
- **반영:** `StayPeriod`의 상한 검사 제거, 테스트 T1b를 "상한 없음"으로 재작성 (1,234박 생성 허용 확인). 반영 후 216개 전부 통과.

아래는 뒤집기 전의 원래 기록이다.

- **원래 결정:** `StayPeriod`는 최대 **30박**이다. 초과하면 400 `INVALID_REQUEST`.
...
### D30. 멱등 키에 실패 완료 상태(`FAILED:{코드}`)를 둔다 — ✅ **동의받음 (관리자, 2026-08-16)**
...
### D31. 예상 못 한 실패(500)의 멱등 키는 삭제한다 — ✅ **동의받음 (관리자, 2026-08-16)**
...
### D32. 결제 승인 후 확정 경합 패배는 전부 보상(환불) 분기로 수렴시킨다 — 🔍 **구현됨, 관리자 재검토 대기 (백로그)**
...
- **관리자 재검토 대기 (2026-08-16):** 관리자가 다른 순서를 제안했다 — **"예약·확정 처리를 먼저 다 하고 → 결제하고 → 성공하면 커밋".** 즉 전이 UPDATE를 트랜잭션 안에서 먼저 걸고(커밋 전), 결제를 부른 뒤, 승인되면 커밋·거절되면 되돌리는 방식이다.
  - **이 방식이 좋아지는 것:** 전이 UPDATE가 예약 행을 잠근 채 결제를 기다리므로, 그 사이 취소·만료가 끼어들 수 없다. **환불 보상 경로 자체가 필요 없어진다** — D32가 닫은 창이 원천적으로 안 생긴다.
  - **치르는 값:** 결제라는 바깥 호출이 열린 트랜잭션 안에 들어온다. 규칙 문서("세션 안에서 원격 호출 금지")와 D5의 전제가 바뀐다. 잠기는 것은 그 예약 행 하나라 재고 경합에는 영향이 없지만, 결제가 느릴 때 DB 커넥션을 그만큼 쥔다 — 부하에서 커넥션 풀 소진 위험이 이쪽으로 옮겨온다. 실 PG(수백 ms~수 초)냐 모의 결제(즉시)냐에 따라 값이 크게 다르다.
  - **판단 자료 위치:** 현행 구현은 `app/reservation/application/usecases/transition_reservation.py`의 `ConfirmReservationUseCase`, 관련 규칙은 `coding-rules.md`의 「세션 안에서 하면 안 되는 것」·「무엇이 안이고 무엇이 밖인가」 표다.

### D33. 확정에도 소유자 검증을 둔다 — ✅ **관리자 지시 (2026-08-16)**

- **결정:** 확정 요청의 `X-User-Id`가 예약의 소유자와 다르면 **404**다. 결제는 호출되지 않는다.
- **경위:** 취소·조회는 남의 예약을 404로 막는데 확정만 열려 있었다. 확인번호만 알면 제3자가 남의 결제를 일으킬 수 있는 문이었고, 완료 보고서의 열린 질문으로 올렸더니 관리자가 "당연히 필요하다"고 답했다.
- **체크인·체크아웃은 그대로 열어 둔다:** 프런트 직원이 손님 대신 처리하는 동선이라(2.1절) 소유자 검증이 오히려 업무를 막는다.

### D34. 예약 목록 API는 F05가 제안한 계약을 그대로 받는다 — ✅ **관리자 지시로 재개, 구현 중 결정 (2026-08-16)**

- **결정:** `GET /api/reservations`를 만든다. 계약은 2.1절의 「목록·호텔 API 계약」 — 헤더 `X-User-Id` 필수, 원소는 단건 응답과 같은 형태, 최신순, 선택 쿼리 `?status=`(enum 검증 포함).
- **경위:** 관리자가 F05 세션에 "예약 조회는 user_id로 결제 완료 건을 조회할 수 있어야 한다"고 지시했고, F05가 계약을 제안하며 "병합된 실물 기준으로 맞추겠다"고 했다. 기술적으로 문제가 없어 그대로 채택했다 — 세션끼리 직접 맞추는 규칙 그대로다.
- **응답을 감싸지 않고 배열로 낸 이유:** F05 제안이 배열이었고, 페이지네이션이 필요해지면 그때 감싸는 것이 계약 변경이지만, 이 과제 범위(사용자당 예약 수십 건)에서는 필요가 없다. 뒤집히면 라우터와 스키마 몇 줄이다.
- **트레이드오프:** 페이지네이션·총계가 없어서 예약이 수천 건인 사용자는 응답이 커진다. 이 과제의 사용 패턴에서는 일어나지 않는다.

### D35. 시드를 호텔 100곳으로 확장하되, 기존 호텔 1·2의 계약은 동결한다 — ✅ **관리자 지시 (2026-08-16)**

- **결정:** 리비전 `053`으로 호텔 3~100(98곳)과 호텔당 객실타입 3종, 재고 26,460행을 추가한다. 규칙은 1.9절 (2b)(3b). **호텔 1·2와 객실타입 1~5는 값 하나 바꾸지 않는다.**
- **동결하는 이유:** F04의 부하테스트 시나리오(`docs/load-test/scenarios.md`)와 기존 테스트가 객실타입 3(스위트 10실)·5를 경합 대상 상수로 박아 두었다. 확장이 그 값을 건드리면 남의 소유 문서가 전부 어긋난다.
- **id 규칙을 h×1000+n으로 한 이유:** F05가 규칙만으로 매핑 상수를 생성할 수 있어야 하고, 테스트 전용 id 대역(900~999, 9300번대, 93200번대)과 겹치면 안 된다. 3001~100003은 어느 대역과도 겹치지 않는다.
- **⚠️ F04 문서와의 어긋남 (기록만):** `docs/load-test/` 문서들이 "재고 450행(5타입×90일)"으로 서술한 부분이 이 확장으로 낡았다 — 전체 행 수는 이제 26,910이다. 다만 **F04가 쓰는 객실타입 1~5의 값과 행은 그대로**라 시나리오 자체는 깨지지 않고, 전수 검증 쿼리가 전체 행 수를 세는 경우만 숫자가 달라진다. F04 문서는 남의 소유라 고치지 않고 여기 기록한다.
- **시드 실행 시간:** 리비전 `053` 단독 약 0.2초, 전체 마이그레이션 약 0.5초 (로컬 실측). 재시드 절차(1.9절 (5))는 그대로 유효하다.

### D36. 호텔 목록 API는 익명이고 정적 마스터만 준다 — ✅ **구현 중 결정 (2026-08-16)**

- **결정:** `GET /api/hotels`는 인증 없이 호텔 100곳과 객실타입 매핑을 준다. 잔여 수량은 싣지 않는다. 캐시도 두지 않는다.
- **이유:** F05의 검색 화면이 "호텔 N곳 조회" 구조라 호텔 목록 없이는 100곳을 그릴 수 없다. 잔여는 F03 검색이 이미 답하고 있고, 여기 실으면 같은 질문에 두 API가 다른 시점의 답을 주게 된다. 캐시는 시드 이후 바뀌지 않는 데이터에 무효화 경로만 하나 늘린다.
- **트레이드오프:** 요청마다 299행 조인 한 발이 나간다. 호텔 수가 수만이 되면 캐시가 필요해지지만, 그때 붙여도 계약은 안 바뀐다.

### D37. 컨테이너 싱글턴은 전부 잠금 있는 판(ThreadSafeSingleton)으로 만든다 — ✅ **구현 중 결정 (기록, 2026-08-16)**

**F04 부하테스트가 실측으로 잡아 넘긴 문제를 닫는 결정입니다** (되돌리는 비용이 코드 몇 줄이라 정하고 진행하며 기록). 번호는 D34~D36과 겹치지 않게 D37로 둔다 — 같은 날 다른 브랜치에서 D34가 먼저 쓰였다.

- **문제:** 앱을 새로 띄운 직후 첫 요청이 수백 개 동시에 오면 5xx가 터졌다 (F04 실측: MySQL "Too many connections" 21건, 워커 하나가 커넥션 44개). 조립기의 `providers.Singleton`은 "있나 보고 → 없으면 만든다" 사이에 잠금이 없어서, 차가운 상태의 스레드풀 스레드 수십 개가 저마다 DB 엔진을 만들었다.
- **결정:** 두 컨테이너의 싱글턴 전부를 `ThreadSafeSingleton`으로 바꾼다. 만들기는 앱 수명에 몇 번뿐이라 잠금 비용은 없는 것과 같다.
- **검증:** 테스트 T90(스레드 16개 동시 첫 접근 → 엔진 정확히 1개; 고치기 전엔 16개가 만들어졌다)과 T90b(잠금 없는 Singleton이 하나라도 남으면 빨간불이 되는 구조 검사).
- **대안이었던 것:** 앱 시작 시 미리 만들어두기(lifespan 선초기화) — 증상은 사라지지만 조립기 자체는 여전히 스레드 불안전이라 새 싱글턴이 같은 문제를 재발시킨다. 근본을 고치는 쪽을 택했다.

---

# 부록 B. 열린 질문

- ~~확정·체크인·체크아웃의 소유자 검증 비대칭~~ — **해소됨 (D33).** 확정은 소유자 검증 404, 체크인·체크아웃은 프런트 대리 처리 동선이라 그대로 둔다.
