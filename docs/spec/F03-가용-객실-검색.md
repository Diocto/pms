...
**F01의 `app/inventory/container.py`는 건드리지 않는다.** 같은 컨텍스트 이름을 쓰지만 F03은 자기 슬라이스의 `app/inventory/query/container.py`를 따로 두고 루트에서 직접 묶는다. 컨텍스트 컨테이너 파일 하나를 두 세션이 함께 고치는 상황을 만들지 않기 위해서다.
...
**오류 본문 형태는 `app/common/response.py`의 `ErrorResponse`(`code`·`message`·`traceId`)와 일치한다.** 다만 **`code` 문자열 하나가 공통 기본값과 다르다.**

| 상황 | 공통 예외의 기본 `code` | F03이 내보내는 `code` |
|---|---|---|
| 잘못된 요청 | `InvalidRequestError` → `INVALID_REQUEST` | `INVALID_REQUEST` (같다) |
| 호텔 없음 | `NotFoundError` → **`NOT_FOUND`** | **`RESOURCE_NOT_FOUND`** |

**`NotFoundError`를 그대로 던지면 F04가 못 받는 문자열이 나간다.** `app/common/errors.py`가 "각 컨텍스트는 셋 중 하나를 상속해 `code`만 바꾼다"고 정해뒀으므로, F03은 `HotelNotFoundError(NotFoundError)`에 `code = "RESOURCE_NOT_FOUND"`를 둔다. **상속을 빠뜨려도 404는 그대로 나가고 문자열만 달라진다** — `emptyReason` 대소문자와 정확히 같은 종류의 조용한 어긋남이라 9절 TDD가 이 문자열을 직접 확인한다.
...
- **404는 `NotFoundError`를 그대로 던지지 않는다.** 기본 `code`가 `NOT_FOUND`라 계약의 `RESOURCE_NOT_FOUND`와 어긋난다. `code`를 바꾼 하위 예외를 만든다 (8절)
...
5절 계약표 A1~A9는 2026-08-15에 F01로부터 확정본을 받아 값이 다 채워졌다. 다만 그중 셋(**A2·A8, 그리고 A4~A6의 `CHECK` 표현**)은 **F01 안에서만 확정이고 관리자 판단은 아직 없다** — 각각 F01의 D16·D2·D3이며 F01의 [확인] 묶음에 함께 걸려 있다.
...
**전 회차에 적어둔 착수 차단 조건 하나가 해소됐다.** "`app/common/clock.py`가 오지 않으면 멈춘다"였는데, `main` 병합(`1bb47f9`)으로 그 파일이 실제로 들어왔다. **지금 F03의 착수를 막는 조건은 하나뿐이고 그건 F01 병합이다** — 계약표 A1~A9를 실제 DDL로 대조해야 하고, C6이 F01의 예약 생성에 의존한다.

**규칙 변경으로 소유 범위 항목이 하나 늘었다.** 캐시 설정 2줄(`app/common/config.py`)과 컨테이너 배선 1줄(`app/containers.py`)이 필요하다. 둘 다 `parallel-work.md`의 "절차를 밟으면 고칠 수 있다" 목록에 있고 자기 줄만 추가하는 형태라 충돌 위험은 낮지만, **전 회차에 "수정이 필요한 공유 파일: 없음"이라고 적어 승인받으려 했으므로 이번 재승인에서 정정한다** (3절).
