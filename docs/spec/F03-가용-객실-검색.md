...
**이 두 줄은 F01이 `errors.py`를 고친 커밋 뒤에 올린다.** 같은 공유 파일을 같은 방식(자기 줄만 추가)으로 만지는 커밋이 둘 있고, 순서를 정해두면 자동 병합이 흔들릴 자리가 없다. 순서는 F01 → F03이다.
...
│   └── actuator.py         캐시 on/off 현재 값 노출 (D15)
...
**층 이름이 바뀌면서 D15가 갈 자리가 정해졌다.** 캐시 스위치 노출은 HTTP로 나가는 운영 엔드포인트이므로 `presentation/actuator.py`다. 값을 읽어오는 곳(설정 프로바이더)과 값을 내보내는 곳이 다른 층이라는 게 이제 경로에 드러난다.
...
### 운영 엔드포인트 — 캐시 스위치 현재 값 (D15)

| 메서드 | 경로 | 설명 |
|---|---|---|
| GET | `/internal/config/search-cache` | 지금 실제로 쓰이는 캐시 설정 값 |
...
**바깥 키는 `camelCase`, 안쪽 키는 환경변수 이름 그대로다.** `coding-rules.md`가 "설정 키 표기 규칙이 API JSON 표기 규칙보다 우선한다"로 못 박은 예외다. 이유는 **이 응답만 사람이 읽고 셸에 그대로 옮겨 적는 응답**이기 때문이다 — 여기 적힌 문자열을 그대로 `PMS_SEARCH_CACHE_ENABLED=false uvicorn app.main:app`에 붙여넣을 수 있어야 한다. 표기를 바꾸면 그 옮겨 적기가 깨지고, 그건 편의가 아니라 재현성 문제다.

**`implementation`은 확인의 핵심이다.** 설정 값만 보여주면 "값은 `false`인데 컨테이너가 여전히 Redis 구현을 주입하고 있는" 상태를 못 잡는다. **실제로 주입된 객체의 클래스 이름**을 함께 내보내, 값과 구현이 어긋나면 눈에 보이게 한다. 값과 구현이 같은 프로바이더에서 나온다는 D15의 주장을 이 필드가 검증한다.
...
| 13 | `PMS_SEARCH_CACHE_ENABLED=false`면 항상 DB에서 읽고, **actuator가 노출하는 값도 `false`다** | 유스케이스 | No-op 구현 선택과 **값 노출이 같은 프로바이더를 보는지**. 컨테이너 `override`로 설정을 바꿔 끼운다. **절단 대비의 핵심이고, 보장이 타입에서 조립 설정으로 내려왔으므로 테스트로 고정한다** (D15) |
...
| 20b | 운영 엔드포인트가 설정 값과 실제 주입된 구현을 함께 내보낸다 | API | D15. 응답의 키가 `PMS_SEARCH_CACHE_ENABLED` 그대로인지(설정 키 예외), `implementation`이 컨테이너가 실제로 준 클래스 이름인지를 본다. **값만 맞고 구현이 다른 상태를 잡는 것이 이 테스트의 목적이다** |
...
- **결정:** 설정 키를 **`PMS_SEARCH_CACHE_ENABLED`**(기본 `true`)로 두고, **캐시 구현을 고르는 쪽과 그 값을 노출하는 쪽이 같은 프로바이더를 주입받는다.** 값은 `presentation/actuator.py`의 `GET /internal/config/search-cache`로 노출해 F04가 실행 전에 확인할 수 있게 한다. 응답에는 설정 값과 함께 **실제로 주입된 구현 클래스 이름**을 싣는다 (8절).
...
- **자리가 규칙으로 정해졌다.** 층 개명(`adapter` → `presentation` + `infrastructure`) 이후 이 엔드포인트는 **HTTP로 나가는 들어오는 경계**이므로 `presentation/actuator.py`다. `clean-architecture.md`의 모듈 예시에도 `presentation/actuator.py`가 그 용도로 들어 있다. 그리고 **응답의 키 표기는 `camelCase` 규칙의 명시적 예외**다 — 설정 키는 조작자가 셸에 옮겨 적는 문자열이라 대문자·밑줄 그대로 둔다.
- **트레이드오프:** 운영 엔드포인트가 하나 늘고, F03의 소유 범위에 `presentation/actuator.py`가 추가된다. 노출 값이 하나뿐이라 비용은 작지만 **"조회 전용 슬라이스"라는 F03의 성격에 운영 관심사가 조금 섞인다.** 그리고 응답 표기가 다른 엔드포인트와 달라 보이는데, 이건 규칙이 의도한 예외라 스펙에 명시해 둔다.
- **대안이었던 것:** 노출하지 않고 F04가 로그로 추정하기(추정은 검증이 아니고, 틀리면 "캐시가 없어도 같더라"는 정반대 결론을 만든다). 설정을 F01의 운영 엔드포인트에 얹기(F01 소유 침범).
- **확인 질문:** 캐시 on/off 값을 F03이 직접 노출하는 것에 동의합니까? 키 이름을 `PMS_SEARCH_CACHE_ENABLED`로 두는 것에 동의합니까?
...
**규칙 변경으로 소유 범위 항목이 하나 늘었다.** 캐시 설정 2줄(`app/common/config.py`)과 컨테이너 배선 1줄(`app/containers.py`)이 필요하다. 둘 다 `parallel-work.md`의 "절차를 밟으면 고칠 수 있다" 목록에 있고 자기 줄만 추가하는 형태라 충돌 위험은 낮지만, **전 회차에 "수정이 필요한 공유 파일: 없음"이라고 적어 승인받으려 했으므로 이번 재승인에서 정정한다** (3절). 커밋 순서는 F01의 `errors.py` 수정 뒤다.
