...
**토큰은 정상이고 프로젝트에도 붙었는데, 그 이름의 서비스가 없다는 뜻이다.** 실제로 토큰만 먼저 넣고 서비스를 안 만든 상태에서 이 오류가 났다.

둘 중 하나를 한다.

1. Railway에 `pms-api`·`pms-web` 이름으로 서비스를 만든다
2. 이미 다른 이름으로 만들었다면 저장소 변수 `RAILWAY_BACKEND_SERVICE`·`RAILWAY_FRONTEND_SERVICE`에 실제 이름을 넣는다

CD 로그의 「연결 확인」 단계가 지금 연결된 프로젝트와 기대하는 서비스 이름을 함께 찍는다. 이름이 어긋났는지 거기서 본다.

### 빌드는 되는데 컨테이너가 바로 죽는다

`PMS_DATABASE_URL` 또는 `PMS_REDIS_URL`이 없으면 기동 스크립트가 **일부러** 종료 코드 1로 죽는다. Railway 로그 첫 줄에 어느 변수가 없는지 찍힌다.

### 화면은 뜨는데 `/api` 호출이 500

`PMS_BACKEND_ORIGIN`이 빌드 시점에 없었던 경우다. 값을 넣고 **다시 빌드**해야 한다(2절 참고). 실행만 다시 하는 것으로는 안 고쳐진다.
